# Historical PIPELINE_FOLDER_MAP snapshot - 2026-08-11

This document preserves the complete `PIPELINE_FOLDER_MAP.md` source body from
commit `981501502c49c68b791a96652cd2bd65bb82daaf` before Phase 3C topology-guide
reconciliation and deterministic generation.

It is historical evidence and reference material only. It is non-authoritative:
it does not control normal work, replace `CURRENT_WORKFLOW.md`, replace
`configs/repository_surface.json`, define public commands, grant research
...
The embedded source body records topology classifications and chronology as
they existed at the source commit. This snapshot does not claim that those
embedded classifications remain current, and it does not claim to be the
generated current topology guide. Use `CURRENT_WORKFLOW.md` for normal work and
`configs/repository_surface.json` as the canonical machine-readable
repository-role registry.

Source path: `PIPELINE_FOLDER_MAP.md`
Source commit: `981501502c49c68b791a96652cd2bd65bb82daaf`
...
| `CURRENT_REACHABLE` | `src/futures_rebuild/micro_alpha_pipeline.py`; `src/futures_rebuild/micro_alpha_phase1b2_preparation.py`; `configs/apex_micro_phase1b2_prepare_only_contract_v1.json` | Exact 2018-2024 scope is preserved as 120 source pairs / 1,232,883,585 compressed DBN bytes and 140 coverage cells; all 120 inactive Phase 1B Parquets are now bound by the successful full Phase 2 certificate | Historical-row authority was consumed once; no rerun, publication, activation, registration, or evaluation authority remains |
