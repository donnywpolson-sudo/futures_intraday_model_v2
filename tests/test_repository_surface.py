...
def test_valid_registry_loads_and_validates_current_checkout() -> None:
    ...
    assert len(surface["entries"]) == (
        212 if surface.get("current_direct_authority_registry_id") else 189
    )
...
@pytest.mark.parametrize("successor_count", [211, 213])
def test_direct_authority_registry_count_rejects_non_212(successor_count: int) -> None:
    surface = _surface()
    if successor_count == 211:
        surface["entries"] = [
            entry
            for entry in surface["entries"]
    ...
    with pytest.raises(RepositorySurfaceError, match="registry entry count must remain 212"):
...
def test_default_cli_reports_all_generated_surface_validity() -> None:
    ...
    expected_entry_count = (
        212 if _surface().get("current_direct_authority_registry_id") else 189
    )
