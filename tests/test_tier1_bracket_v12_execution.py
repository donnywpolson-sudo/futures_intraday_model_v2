...
from futures_rebuild.tier1_bracket_v10_execution import V10ExecutionResult
from futures_rebuild.tier1_bracket_v12_execution import (
    EXECUTION_OPERATION_V12, authorized_source_streams_v12,
    build_evidence_manifest_v12, claim_historical_operation_receipt_v12,
)


def _receipt(
    *, boundary: RepoBoundary, trial_id: str, source_id: str,
    output: Path, plan_id: str, plan_sha: str,
) -> OperationReceipt:
    scope = {
        "trial_id": trial_id, "source_binding_id": source_id,
        "output_root": output.as_posix(),
        "holdout_or_forward_access": "false", "provider_access": "false",
        "publication": "false",
    }
    return OperationReceipt.issue_user_approved(
        boundary, operation=EXECUTION_OPERATION_V12,
        classification=OperationClassification.EXTERNAL_REAL_HISTORY_AUTHORIZATION,
        scope=scope, approval_command=EXECUTION_OPERATION_V12,
        approval_plan_id=plan_id, approval_plan_sha256=plan_sha,
        approval_line=f"APPROVE {EXECUTION_OPERATION_V12} PLAN {plan_id} SHA256 {plan_sha}",
    )


def test_v12_historical_authorization_is_exact_durable_and_single_use(tmp_path: Path) -> None:
    boundary = RepoBoundary(tmp_path)
    trial_id, source_id, plan_id, plan_sha = ("a" * 64, "b" * 64, "c" * 64, "d" * 64)
    output = tmp_path / "unpublished"
    receipt = _receipt(
        boundary=boundary, trial_id=trial_id, source_id=source_id,
        output=output, plan_id=plan_id, plan_sha=plan_sha,
    )
    ...
    with pytest.raises(UnauthorizedOperation, match="already consumed"):
        claim_historical_operation_receipt_v12(
            root=tmp_path, boundary=boundary, receipt=receipt,
            trial_id=trial_id, source_binding_id=source_id, output_root=output,
            plan_id=plan_id, plan_sha256=plan_sha,
        )
...
def test_v12_rejects_2025_before_registry_or_file_open(tmp_path: Path) -> None:
    boundary = RepoBoundary(tmp_path)
...
def test_v12_manifest_binds_full_evidence_runtime_source_and_authority() -> None:
    runtime = {
    ...
    manifest = build_evidence_manifest_v12(trial_id="f" * 64, result=result)
    assert len(manifest["files"]) == 10
    assert {"runtime_receipt.json", "source_integrity_audit.json"} <= set(manifest["files"])
