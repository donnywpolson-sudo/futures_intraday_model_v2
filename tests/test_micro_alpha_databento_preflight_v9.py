...
def test_v9_preserves_v8_failure_and_binds_exact_scope(tmp_path: Path) -> None:
    ...
    assert plan["correction"]["reason"] == (
        "PRELAUNCH_DISCOVERY_REQUIRES_TWO_STAGE_SYMBOLOGY_PROOF"
    )
...
def test_v9_accepts_only_exact_prelaunch_discovery_and_empty_verification() -> None:
    discovery = _symbology_summary(
        _response(partial=["MES.FUT"], not_found=[]),
        symbol="MES.FUT",
        stype_in="parent",
    ...
    for field, value in (("partial", ["OTHER"]), ("not_found", ["MES.FUT"])):
        payload = _response(partial=["MES.FUT"], not_found=[])
        payload[field] = value
...
def test_v9_successful_metadata_mechanics_are_price_free_and_download_free(
    tmp_path: Path,
) -> None:
    root = _copy_root(tmp_path)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([], False, "SYMBOL_STATUS_SHAPE_DRIFT", 4),
        (["MES.FUT"], [], "PARTIAL_OR_NOT_FOUND_SYMBOLOGY", 5),
        ([], ["MES.FUT"], "PARTIAL_OR_NOT_FOUND_SYMBOLOGY", 4),
    ],
)
def test_v9_status_shape_and_resolution_failures_are_sanitized(
    tmp_path: Path,
    partial: object,
    not_found: object,
    failure_code: str,
...
def test_v9_message_drift_reports_only_the_field_name(tmp_path: Path) -> None:
    root = _copy_root(tmp_path)
...
def test_v9_nonzero_cost_and_unresolved_pre_dataset_date_fail_closed(
    tmp_path: Path,
) -> None:
...
def test_v9_requires_exact_authorization_and_create_only_output(tmp_path: Path) -> None:
    root = _copy_root(tmp_path)
    plan = json.loads((root / PLAN_PATH).read_text(encoding="utf-8"))
