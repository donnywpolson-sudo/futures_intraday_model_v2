...
from futures_rebuild.tier1_bracket_v11_execution import (
    EXECUTION_OPERATION_V11, authorized_source_streams_v11,
    build_evidence_manifest_v11, claim_historical_operation_receipt_v11,
)
...
def _receipt(
    *, boundary: RepoBoundary, trial_id: str, source_id: str,
    ...
    return OperationReceipt.issue_user_approved(
        ...
        approval_line=f"APPROVE {EXECUTION_OPERATION_V11} PLAN {plan_id} SHA256 {plan_sha}",
    )


def test_v11_historical_authorization_is_exact_durable_and_single_use(tmp_path: Path) -> None:
    ...
    with pytest.raises(UnauthorizedOperation, match="already consumed"):
        claim_historical_operation_receipt_v11(
            root=tmp_path, boundary=boundary, receipt=receipt,
            trial_id=trial_id, source_binding_id=source_id, output_root=output,
            plan_id=plan_id, plan_sha256=plan_sha,
        )
...
def test_v11_rejects_2025_before_registry_or_file_open(tmp_path: Path) -> None:
    boundary = RepoBoundary(tmp_path)
...
def test_v11_manifest_binds_full_evidence_runtime_source_and_authority() -> None:
    runtime = {
    ...
    manifest = build_evidence_manifest_v11(trial_id="f" * 64, result=result)
