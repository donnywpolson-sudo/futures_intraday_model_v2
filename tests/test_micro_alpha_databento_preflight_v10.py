...
from futures_rebuild.micro_alpha_databento_preflight_v10 import (
...
def test_v10_preserves_v9_failure_and_binds_exact_scope(tmp_path: Path) -> None:
    root = _copy_root(tmp_path)
    failure = load_predecessor_failure(root=root)
    plan = build_plan(root=root)
    validate_plan(plan, root=root)
    assert failure["report_id"] == PREDECESSOR_REPORT_ID
    assert plan["correction"]["reason"] == "PROVIDER_PARTIAL_ENTRY_CONTENT_IS_OPAQUE"
...
def test_v10_accepts_one_opaque_discovery_partial_only_under_exact_bindings() -> None:
    opaque_entry = "OPAQUE_PROVIDER_ENTRY_MUST_NOT_BE_REPORTED"
    discovery = _symbology_summary(
        _response(partial=[opaque_entry], not_found=[]),
        symbol="MES.FUT",
        stype_in="parent",
    ...
    assert discovery["partial_count"] == 1
    ...
    assert verification["partial_count"] == verification["not_found_count"] == 0
    for field, value in (
        ("partial", False),
        ("partial", {}),
        ("partial", [1]),
    ...
    for field, value in (
        ("partial", ["OPAQUE_A", "OPAQUE_B"]),
        ("not_found", ["MES.FUT"]),
    ):
        payload = _response(partial=[opaque_entry], not_found=[])
        payload[field] = value
...
def test_v10_successful_metadata_mechanics_are_price_free_and_download_free(
    tmp_path: Path,
) -> None:
    root = _copy_root(tmp_path)
    ...
    for market in ("MES", "MCL", "MGC", "M6E"):
        ...
        assert discovery["partial_count"] == 1
        assert discovery["not_found_count"] == 0
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["OPAQUE_A", "OPAQUE_B"], [], "PARTIAL_OR_NOT_FOUND_SYMBOLOGY", 4),
        ([], ["MES.FUT"], "PARTIAL_OR_NOT_FOUND_SYMBOLOGY", 4),
    ],
)
def test_v10_status_shape_and_resolution_failures_are_sanitized(
    tmp_path: Path,
    partial: object,
    not_found: object,
    failure_code: str,
...
def test_v10_message_drift_reports_only_the_field_name(tmp_path: Path) -> None:
    root = _copy_root(tmp_path)
...
def test_v10_nonzero_cost_and_unresolved_pre_dataset_date_fail_closed(
    tmp_path: Path,
) -> None:
...
def test_v10_requires_exact_authorization_and_create_only_output(tmp_path: Path) -> None:
    root = _copy_root(tmp_path)
    plan = json.loads((root / PLAN_PATH).read_text(encoding="utf-8"))
