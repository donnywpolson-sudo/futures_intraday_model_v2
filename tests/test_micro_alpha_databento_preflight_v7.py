...
def _response(*, partial: object, not_found: object) -> dict[str, object]:
    return {
...
def test_v7_preserves_v6_failure_and_binds_exact_scope(tmp_path: Path) -> None:
    ...
    assert plan["correction"]["reason"] == (
        "DATABENTO_SYMBOLOGY_STATUS_FIELDS_ARE_JSON_LISTS"
    )
...
def test_v7_accepts_only_exact_empty_status_lists() -> None:
...
def test_v7_successful_metadata_mechanics_are_price_free_and_download_free(
    tmp_path: Path,
) -> None:
    root = _copy_root(tmp_path)
...
def test_v7_status_shape_and_resolution_failures_are_sanitized(
    tmp_path: Path,
    partial: object,
    not_found: object,
    failure_code: str,
...
def test_v7_nonzero_cost_and_unresolved_pre_dataset_date_fail_closed(
    tmp_path: Path,
) -> None:
...
def test_v7_requires_exact_authorization_and_create_only_output(tmp_path: Path) -> None:
    root = _copy_root(tmp_path)
    plan = json.loads((root / PLAN_PATH).read_text(encoding="utf-8"))
