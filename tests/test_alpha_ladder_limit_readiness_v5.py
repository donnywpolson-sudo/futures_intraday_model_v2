...
from futures_rebuild import alpha_ladder_limit_readiness_v5 as v5
from tests.test_alpha_ladder_limit_readiness_v2 import _bars


def test_v5_runs_the_entire_adapter_and_evidence_chain(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(v5, "load_plan", lambda *, root: plan)
    monkeypatch.setattr(v5, "PLAN_PATH", plan_path)
    observed = {}
    ...
    assert v5.execute_once(root=tmp_path, boundary=object(), receipt=object()) == {"ok": True}
    assert observed["scope"]["output_root"] == v5.OUTPUT_ROOT.as_posix()
