...
| `PREPARED_NOT_EXECUTED` | `configs/apex_micro_tier01_databento_metadata_preflight_v14.json`; `src/futures_rebuild/micro_alpha_databento_preflight_v14.py` | Exact-scope immutable v14 provider-result-group-safe successor retains exact request echoes and accepts only bounded market-root-consistent groups with fully validated positive instrument IDs and bounded mapping intervals, without recording group keys or identity values; all entitlement, partial/not-found, cost, byte, disk and collision gates remain; 20 definitions and a 375-call/$0/zero-retry ceiling with no download surface | Exact staging, commit, and new single-use metadata/provider approval required |
| `PREPARED_NOT_EXECUTED` | `src/futures_rebuild/micro_alpha_acquisition.py` | Phase 1A annual market-year create-only inactive-custody downloader implemented and adversarially tested; canonical `data/dbn/<schema>/<market>/<year>/` tree only; at most two isolated download clients, stop-after-first-failure scheduling, and no DBN output | Separate download approval absent; passing v14 preflight and committed HEAD required first |
...
PROJECT_OUTLINE design
  ...
  -> immutable v14 provider-result-group-safe successor            PREPARED_NOT_EXECUTED / approval absent
...
No micro phase is labeled complete. The v2, v4, v5, v6, v7, v8, v9, v10, v11, v12, and v13 metadata-only attempts
executed once each and produced only preserved fail-closed metadata evidence.
The v14 successor and Phase 1A downloader are implemented but unexecuted. Phase
...
from the preserved price-free v2, v4, v5, v6, v7, v8, v9, v10, v11, v12, and v13 failure reports, the code proves mechanics
