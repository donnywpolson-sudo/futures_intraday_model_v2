...
| `PREPARED_NOT_EXECUTED` | `src/futures_rebuild/micro_alpha_phase1b2_phase2_successor.py`; `scripts/prepare_apex_micro_phase1b2_phase2_successor_v4.py`; exact operation `BUILD_APEX_MICRO_PHASE2_CERTIFIED_INACTIVE_SUCCESSOR_V4_ONCE` | Stat-only planner binds 120 Phase 1B Parquets / 6,627,486,838 bytes, 140 coverage cells and 24 causal outputs; execution may scan each source, classify every definition repeat set, certify interval/cross-year roll continuity and create inactive Phase 2/certification evidence | Requires implementation commit, immutable plan/audit and separate full derived-row approval; maximum 144 Parquet opens, two workers, 100,000-row batches, 64 GiB output, 12 hours; no DBN, 2025/2026, active mutation, publication, registration or evaluation |
