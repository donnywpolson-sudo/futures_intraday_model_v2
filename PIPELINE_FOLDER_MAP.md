...
| `PREPARED_NOT_EXECUTED` | `src/futures_rebuild/micro_alpha_phase1b2_decoder.py`; `src/futures_rebuild/micro_alpha_phase1b2_execution.py`; `scripts/prepare_apex_micro_phase1b2_execution_v1.py` | Offline micro-only decoder/executor implements five schemas, exact annual source checks, 100,000-row batches, two-worker stop scheduling, write-time 64 GiB ceiling, explicit dispositions, inactive Phase 1B/2 outputs, certification, terminal-last evidence, and collision-checked 96-bit path aliases with full 256-bit identities retained in records | V2 hashed eligible bytes but completed zero decodes; v3 requires a committed exact plan plus fresh confirmation |
| `HISTORICAL_ROW_APPROVAL_REQUIRED` | `src/futures_rebuild/micro_alpha_pipeline.py`; `src/futures_rebuild/micro_alpha_phase1b2_preparation.py`; `configs/apex_micro_phase1b2_prepare_only_contract_v1.json` | Exact 2018-2024 scope is 120 source pairs / 1,232,883,585 compressed bytes and 140 coverage cells; decoder roles, identity/economics, rank-zero roll continuity, causal availability, honest one-second semantics, inactive catalog certification, lane-aware gateway, and fail-closed micro risk gates are bound | Phase 1B/2 row processing has not executed; final immutable post-commit plan and separate exact historical-row confirmation required |
...
| `PREPARED_NOT_EXECUTED` | `src/futures_rebuild/micro_alpha_phase1b2_phase2_diagnostic.py`; `scripts/prepare_apex_micro_phase1b2_phase2_diagnostic_v1.py`; exact operation `DIAGNOSE_APEX_MICRO_PHASE2_FIRST_INTERVAL_V1_ONCE` | Footer-only planner binds one exact inactive M6E 2018 one-minute Phase 1B Parquet; authorized executor may materialize only that interval under one worker, five minutes, 1 GiB, one attempt, zero retries, and price-free reporting | Requires commit, immutable diagnostic plan/audit, and separate derived-row approval; no DBN or second Parquet source is reachable |
| `NOT_IMPLEMENTED` | Full Phase 1B/2 successor plan after diagnostic | Successor design waits for the bounded Phase 2 diagnostic result | No current full row-read execution authority exists |
...
custody is now complete. Phase 1B/2 contracts are prepared, but row processing
is unexecuted. The micro catalog is inactive, registration is blocked, and all
