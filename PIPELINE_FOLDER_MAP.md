...
| `PREPARED_NOT_EXECUTED` | `src/futures_rebuild/micro_alpha_phase1b2_definition_duplicate_diagnostic.py`; `scripts/prepare_apex_micro_phase1b2_definition_duplicate_diagnostic_v3.py`; exact operation `DIAGNOSE_APEX_MICRO_DEFINITION_DUPLICATE_SEMANTICS_V3_ONCE` | Stat-only planner binds only the 68,274-byte M6E 2018 definition Parquet and sealed group result; the executor may report counts distinguishing identical retained semantics from distinct same-key updates | Requires implementation commit, immutable plan/audit, and separate one-file derived-row approval; no raw values/keys, Parquet output, DBN, second Parquet, or 2025/2026 access |
| `NOT_IMPLEMENTED` | Full Phase 2/certification successor plan after definition classification | Successor design waits for the bounded definition repeat-semantics result; the current duplicate rule remains fail-closed | No current full row-read execution authority exists |
