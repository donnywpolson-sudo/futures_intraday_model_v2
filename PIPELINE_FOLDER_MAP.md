...
| `PREPARED_NOT_EXECUTED` | `configs/apex_micro_tier01_databento_metadata_preflight_v16.json`; `src/futures_rebuild/micro_alpha_databento_preflight_v16.py` | Exact-scope immutable v16 bounded interval-overlap-safe successor requires exact response echoes and exact `d0`/`d1`/`s` entry fields, positive identities, positive ISO ranges, query overlap, and a gap-free union after coverage-only clipping to the exact query; wholly outside, malformed, duplicate, gapped, unrelated, nonzero-cost, disk, and collision cases fail closed; 20 definitions and a 375-call/$0/zero-retry ceiling with no download surface | Exact staging, commit, and new single-use metadata/provider approval required |
| `PREPARED_NOT_EXECUTED` | `src/futures_rebuild/micro_alpha_acquisition.py` | Phase 1A annual market-year create-only inactive-custody downloader implemented and adversarially tested; canonical `data/dbn/<schema>/<market>/<year>/` tree only; at most two isolated download clients, stop-after-first-failure scheduling, and no DBN output | Separate download approval absent; passing v16 preflight and committed HEAD required first |
...
PROJECT_OUTLINE design
  ...
  -> immutable v16 bounded interval-overlap-safe successor         PREPARED_NOT_EXECUTED / approval absent
...
The v16 successor and Phase 1A downloader are implemented but unexecuted. Phase
