...
| `PREPARED_NOT_EXECUTED` | `src/futures_rebuild/research_gateway_policy.py`; exact operation `BUILD_APEX_MICRO_PHASE1B2_INACTIVE_FOUNDATION_V1_ONCE` | Bounded remediation adds only the exact Phase 1B/2 inactive-foundation operation to the central preparatory allowlist; unknown aliases still fail closed | Requires commit, immutable v2 plan/audit, and fresh historical-row confirmation |
| `NOT_IMPLEMENTED` | `configs/apex_micro_phase1b2_historical_execution_plan_v2.json`; `state/unpublished_evidence/apex_micro_phase1b2_execution_plan_v2/audit.json` | Create-only successor plan/audit intentionally wait for the committed central-policy remediation HEAD | No current row-read execution authority exists |
