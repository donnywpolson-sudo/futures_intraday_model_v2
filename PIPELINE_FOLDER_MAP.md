...
| `PREPARED_NOT_EXECUTED` | `configs/apex_micro_tier01_databento_metadata_preflight_v17.json`; `src/futures_rebuild/micro_alpha_databento_preflight_v17.py` | Exact-scope immutable v17 bounded opaque-group-key successor binds expanded provider groups through exact request echoes, permits only bounded printable ASCII group identifiers without recording values, and retains exact mapping fields, positive identities, interval overlap/clipping, gap proof, ceilings, `not_found`, cost, disk, and collision gates; 20 definitions and a 375-call/$0/zero-retry ceiling with no download surface | Exact staging, commit, and new single-use metadata/provider approval required |
| `PREPARED_NOT_EXECUTED` | `src/futures_rebuild/micro_alpha_acquisition.py` | Phase 1A annual market-year create-only inactive-custody downloader implemented and adversarially tested; canonical `data/dbn/<schema>/<market>/<year>/` tree only; at most two isolated download clients, stop-after-first-failure scheduling, and no DBN output | Separate download approval absent; passing v17 preflight and committed HEAD required first |
...
PROJECT_OUTLINE design
  ...
  -> immutable v17 bounded opaque-group-key successor              PREPARED_NOT_EXECUTED / approval absent
...
The v17 successor and Phase 1A downloader are implemented but unexecuted. Phase
