"""Execution boundary for the terminal, state-neutral authoritative trial."""
...
INVALID_TRIAL_ID = (
    "140f865ab06e68b42ab0aaa9a392bb52d12eecf696b738e7bdd40831c0e6d285"
)
INVALID_RETIREMENT_ROOT = Path(
    "state/trial_registry/tier1_authoritative_postcheck_invalid_retirement"
)
INVALID_DISPOSITION = "INVALID_PRE_DATA_POSTACTIVATION_PREPUBLICATION_BUILDER_TEST_DEFECT"
...
def _object(path: Path) -> dict[str, object]:
    try:
        ...
        raise IntegrityError(f"invalid terminal authoritative artifact: {path}") from exc
    if not isinstance(value, dict):
        raise IntegrityError("terminal authoritative artifact is not an object")
    return value


def validate_terminal_registered_context_documents(
    *,
    ...
    if (
        ...
        raise UnauthorizedOperation("terminal authoritative context is inconsistent")
    return trial_id


def load_terminal_registered_context(
    *, root: Path, plan: Mapping[str, object]
    ...
    if not isinstance(trial_id, str) or len(trial_id) != 64:
        raise UnauthorizedOperation("terminal authoritative pointer is unavailable")
    ...
    if not isinstance(retirement_id, str) or len(retirement_id) != 64:
        raise UnauthorizedOperation("terminal invalid-retirement identity is absent")
    ...
    if (
        ...
        raise UnauthorizedOperation("terminal authoritative bindings drifted")
    return trial_id, pointer, registry, certificate


def execute_authorized_terminal_authoritative(
    *, root: Path, boundary: RepoBoundary, receipt: OperationReceipt
    ...
    trial_id, _, registry, _ = load_terminal_registered_context(root=root, plan=plan)
