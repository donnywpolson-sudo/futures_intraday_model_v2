"""Exact authorization and immutable evidence controls for V11."""
...
from .tier1_bracket_v11 import load_v11_contract
from .tier1_bracket_v11_pipeline import run_v11_pipeline


V11_REGISTRY_ROOT = Path("state/trial_registry/tier1_bracket_successor_v11")
EXECUTION_OPERATION_V11 = "EXECUTE_TIER1_BRACKET_SUCCESSOR_V11_HISTORICAL_SCREEN"
PUBLICATION_OPERATION_V11 = "PUBLISH_TIER1_BRACKET_SUCCESSOR_V11_EVIDENCE"
...
def _load(path: Path) -> dict[str, object]:
    try:
        ...
        raise IntegrityError(f"invalid V11 execution artifact: {path.as_posix()}") from exc
    if not isinstance(value, dict):
        raise IntegrityError("V11 execution artifact is not an object")
    return value


def _verify_receipt_v11(
    *, boundary: RepoBoundary, receipt: OperationReceipt, operation: str,
    ...
    if (
        ...
        raise UnauthorizedOperation("V11 operation requires an exact single-use receipt")
    return receipt.receipt_id


def claim_historical_operation_receipt_v11(
    *, root: Path, boundary: RepoBoundary, receipt: OperationReceipt,
    ...
    if not all(v5._hex64(value) for value in (trial_id, source_binding_id, plan_id, plan_sha256)):
        raise UnauthorizedOperation("V11 historical receipt scope is invalid")
    boundary.assert_active_path(output_root.absolute(), purpose="V11 historical output root")
    required = {
        "trial_id": trial_id, "source_binding_id": source_binding_id,
        "output_root": output_root.as_posix(),
        "holdout_or_forward_access": "false", "provider_access": "false",
        "publication": "false", "approval_command": EXECUTION_OPERATION_V11,
        "approval_plan_id": plan_id, "approval_plan_sha256": plan_sha256,
    }
    receipt_id = _verify_receipt_v11(
        boundary=boundary, receipt=receipt, operation=EXECUTION_OPERATION_V11,
        required=required,
    )
    claim = root / "state/authorization_uses" / f"{receipt_id}.json"
    boundary.assert_active_path(
        claim.absolute(), purpose="V11 authorization use",
    ...
    try:
        with claim.open("xb") as stream:
            stream.write(canonical_bytes({
                "schema_version": "tier1_bracket_v11_authorization_use/1.0.0",
        ...
        raise UnauthorizedOperation("V11 historical receipt was already consumed") from exc
    return claim


def authorized_source_streams_v11(
    *, root: Path, boundary: RepoBoundary, receipt: OperationReceipt,
    ...
    registry = _load(root / V11_REGISTRY_ROOT / f"{trial_id}.json")
    ...
    if not isinstance(raw, list) or not all(isinstance(item, dict) for item in raw):
        raise IntegrityError("registered V11 source bindings are absent")
    ...
    if registry.get("source_binding_id") != binding_id or set(source_paths) != set(expected):
        raise IntegrityError("V11 source binding or path map is inconsistent")
    claim = claim_historical_operation_receipt_v11(
    ...
    for key, path in source_paths.items():
        if sha256_file(path) != expected[key]:
            raise IntegrityError("V11 source bytes differ from registration")
...
def execute_authorized_v11(
    *, root: Path, boundary: RepoBoundary, receipt: OperationReceipt,
    trial_id: str, source_paths: Mapping[tuple[str, int], Path], output_root: Path,
    plan_id: str, plan_sha256: str,
) -> V10ExecutionResult:
    streams, audits, claim = authorized_source_streams_v11(
        root=root, boundary=boundary, receipt=receipt, trial_id=trial_id,
...
def build_evidence_manifest_v11(
    *, trial_id: str, result: V10ExecutionResult,
    ...
    if (
        ...
        raise IntegrityError("V11 evidence lacks identity, runtime, or authorization")
    ...
    core = {
        "schema_version": "tier1_bracket_successor_v11_evidence_manifest/1.0.0",
        "trial_id": trial_id, "files": files,
    }
    return {**core, "manifest_id": sha256_json(core)}


def persist_evidence_bundle_v11(
    *, root: Path, boundary: RepoBoundary, output_root: Path, trial_id: str,
    result: V10ExecutionResult, receipt: OperationReceipt,
    plan_id: str, plan_sha256: str,
) -> dict[str, str]:
    manifest = build_evidence_manifest_v11(trial_id=trial_id, result=result)
    boundary.assert_active_path(output_root.absolute(), purpose="V11 evidence output root")
    required = {
        "trial_id": trial_id, "manifest_id": str(manifest["manifest_id"]),
        "output_root": output_root.as_posix(), "publication": "true",
        "holdout_or_forward_access": "false", "provider_access": "false",
        "approval_command": PUBLICATION_OPERATION_V11,
        "approval_plan_id": plan_id, "approval_plan_sha256": plan_sha256,
    }
    receipt_id = _verify_receipt_v11(
        boundary=boundary, receipt=receipt, operation=PUBLICATION_OPERATION_V11,
        required=required,
    )
    claim = root / "state/authorization_uses" / f"{receipt_id}.json"
    boundary.assert_active_path(
        claim.absolute(), purpose="V11 publication authorization use",
        subtree="state/authorization_uses",
    )
    claim.parent.mkdir(parents=True, exist_ok=True)
    try:
        with claim.open("xb") as stream:
            stream.write(canonical_bytes({
                "schema_version": "tier1_bracket_v11_publication_authorization_use/1.0.0",
                "receipt_id": receipt_id, "trial_id": trial_id,
        ...
        raise UnauthorizedOperation("V11 publication receipt was already consumed") from exc
    destination = output_root / trial_id / str(manifest["manifest_id"])
    if destination.exists():
        raise IntegrityError("V11 evidence publication is create-only")
    ...
    for filename, expected_hash in manifest["files"].items():
        ...
        if sha256_file(path) != expected_hash:
            raise IntegrityError("persisted V11 evidence hash mismatch")
    with (staging / "manifest.json").open("xb") as stream:
        stream.write(canonical_bytes(manifest) + b"\n")
    if destination.exists():
        raise IntegrityError("V11 evidence destination appeared during publication")
