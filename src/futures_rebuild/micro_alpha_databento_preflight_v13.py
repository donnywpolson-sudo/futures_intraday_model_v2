"""SDK-opaque-message-safe Apex micro metadata preflight successor.

The executed v12 plan, consumed authorization, and fail-closed report remain
immutable. Databento's installed SDK requires both ``status`` and ``message``
keys but does not interpret either value when constructing an instrument map.
V13 accepts only bounded scalar shapes for status and a bounded exact string
shape for message, recording only shape classifications. HTTP failures remain
rejected by the SDK, while exact request echoes, result keys, mapping intervals,
prelaunch handling, and every acquisition gate remain fail closed. No
download-capable API is exposed to this executor.
"""
...
PREDECESSOR_PLAN_ID: Final = (
    "ee4a68b8e25988ed57f5d188b0ef9ddb1384b07e61c2a1819cc04b146c41b9ee"
)
PREDECESSOR_PLAN_SHA256: Final = (
    "6d681143c059d3983513be7a1b5df7ca8a20fe8e94f07eaeb39e7ce6660cf913"
)
...
PREDECESSOR_REPORT_SHA256: Final = (
    "d5f489b456f73e35b75cc09de85f9866a7186b26ec7e42f2ce53f675e2c57ba8"
)
PREDECESSOR_AUTHORIZATION_RECEIPT_ID: Final = (
    "167a0c16c165cbbbfd9a6542a8bf1ade6c3d408a035266d7248728d76729eebf"
)
...
PREDECESSOR_AUTHORIZATION_SHA256: Final = (
    "20232c29f6d4388b4a4ec381c4b456b98010e0958be3fafc0786803a856288ae"
)
SCHEMA: Final = "apex_micro_databento_metadata_preflight/13.0.0"
REPORT_SCHEMA: Final = "apex_micro_databento_metadata_preflight_report/13.0.0"
...
def load_predecessor_failure(*, root: Path) -> dict[str, object]:
    """Verify exact v12 plan, failure report, and authorization evidence."""
    ...
    if (
        ...
        raise IntegrityError("v12 metadata preflight evidence was not preserved byte-for-byte")
    plan = _object(plan_path, "v12 metadata preflight plan")
    report = _object(report_path, "v12 metadata preflight report")
    plan_core = dict(plan)
    ...
    if (
        ...
        or report.get("failure_code") != "SYMBOL_SUCCESS_MESSAGE_DRIFT"
        ...
        or report.get("failed_validation_field") != "message"
        ...
        raise IntegrityError("v12 metadata preflight failure evidence drifted")
    return report
...
def build_plan(*, root: Path) -> dict[str, object]:
    ...
    predecessor_plan = _object(
        root / PREDECESSOR_PLAN_PATH, "v12 metadata preflight plan"
    )
    ...
    if requests != predecessor_plan.get("requests"):
        raise IntegrityError("v13 market/schema request definitions drifted from v12")
    core: dict[str, object] = {
        "schema_version": SCHEMA,
        "classification": "PREPARED_METADATA_ONLY_PROVIDER_APPROVAL_REQUIRED",
        "state": "PREPARED_NOT_EXECUTED",
        "operation": OPERATION,
        "lane_id": "apex_integer_micro_13",
        "dataset": DATASET,
        ...
        "predecessor_execution": {
            ...
            "failed_call_inference": {
                "basis": (
                    "SEALED_V12_EXACT_REQUEST_AND_RESPONSE_SHAPE_REACHED_"
                ...
                "stype_in": "parent",
                "local_defect": (
                    "SDK_OPAQUE_SUCCESS_MESSAGE_MISINTERPRETED_AS_VALUE_ALLOWLIST"
                ),
                "affected_field_disposition": (
                    "V12_SANITIZED_MESSAGE_FIELD_NAME_ONLY_VALUE_NOT_RECORDED"
                ),
                "sdk_contract_basis": (
                    "INSTALLED_DATABENTO_INSTRUMENT_MAP_REQUIRES_MESSAGE_KEY_"
                    "BUT_DOES_NOT_INTERPRET_VALUE"
                ),
        ...
        "correction": {
            "reason": "SDK_OPAQUE_SUCCESS_MESSAGE_REQUIRES_SHAPE_NOT_VALUE_GATE",
            ...
            "mapping_success_basis": (
                "EXACT_ECHO_RESULT_KEY_INTERVALS_MESSAGE_PARTIAL_NOT_FOUND_AND_DATE_GATES"
            ),
...
def validate_plan(plan: Mapping[str, object], *, root: Path) -> dict[str, object]:
    ...
    if dict(plan) != expected:
        raise IntegrityError("Apex micro metadata preflight v13 drifted")
...
def load_plan(*, root: Path) -> dict[str, object]:
    return validate_plan(_object(root / PLAN_PATH, "v13 metadata preflight plan"), root=root)
...
def execute_preflight(
    *,
    ...
    """Consume one v13 authorization and run metadata-only operations."""
    ...
    boundary.assert_active_path(
        ...
        purpose="Apex micro v13 metadata report",
        subtree="state/unpublished_evidence/apex_micro_metadata_preflight_v13",
    )
    if report_path.exists():
        raise IntegrityError("Apex micro v13 metadata report is create-only")
