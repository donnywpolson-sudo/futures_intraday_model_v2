"""List-shape-safe Apex micro metadata preflight successor.

The executed v6 plan, consumed authorization, and fail-closed report remain
immutable. V7 corrects the local symbology response validator: Databento's
``partial`` and ``not_found`` fields are JSON lists, not booleans. Empty lists
are required; partial or unresolved symbols still fail closed. No
...
PREDECESSOR_PLAN_ID: Final = (
    "7a662be00ec2fd5f1b037ae3aeda892ea2d3a5d24e3b6fc04bb0b9c355d31a11"
)
PREDECESSOR_PLAN_SHA256: Final = (
    "b102bad53f35e749a24321a2beb256af445cea39bca5458ee7b1ec48fcadb94a"
)
PREDECESSOR_REPORT_ID: Final = (
    "50a5ef1000de4535249448873c9f420bfc3d7bc38894d507aa4a5f1be7575c62"
)
PREDECESSOR_REPORT_SHA256: Final = (
    "8b624afeb3159d162feaff19c385007d0ff03d58e438f0174ff2128689b46aa9"
)
PREDECESSOR_AUTHORIZATION_RECEIPT_ID: Final = (
    "8c91ae15b1fca6b118526b12cf8ffda564004e353f5aa0c8a6f7a1da581e2df3"
)
...
PREDECESSOR_AUTHORIZATION_SHA256: Final = (
    "07f634aab6867ec25e71cdec289db2751f8286d684df3c1a97bb5df88cb72a56"
)
SCHEMA: Final = "apex_micro_databento_metadata_preflight/7.0.0"
REPORT_SCHEMA: Final = "apex_micro_databento_metadata_preflight_report/7.0.0"
...
def load_predecessor_failure(*, root: Path) -> dict[str, object]:
    """Verify the exact v6 failure and its one-use authorization evidence."""
    ...
    if (
        ...
        raise IntegrityError("v6 metadata preflight evidence was not preserved byte-for-byte")
    plan = _object(plan_path, "v6 metadata preflight plan")
    plan_core = dict(plan)
    plan_id = plan_core.pop("plan_id", None)
    report = _object(report_path, "v6 metadata preflight report")
    ...
    if (
        ...
        or report.get("exception_type") != "TypeError"
        ...
        raise IntegrityError("v6 metadata preflight failure evidence drifted")
    return report
...
def build_plan(*, root: Path) -> dict[str, object]:
    ...
    predecessor_plan = _object(
        root / PREDECESSOR_PLAN_PATH, "v6 metadata preflight plan"
    )
    ...
    if requests != predecessor_plan.get("requests"):
        raise IntegrityError("v7 market/schema request definitions drifted from v6")
    core: dict[str, object] = {
        ...
        "predecessor_execution": {
            ...
            "failed_call_inference": {
                "basis": (
                    "SEALED_TYPEERROR_AT_FIRST_SUCCESSFUL_RESOLVE_PLUS_"
                    "INSTALLED_SDK_LIST_FIELD_CONTRACT_AND_SYNTHETIC_REPRODUCTION"
                ),
                "call_ordinal": 4,
                "operation": "resolve",
                "market": "MES",
                "symbol": "MES.FUT",
                "stype_in": "parent",
                "local_defect": "UNHASHABLE_JSON_LIST_TESTED_WITH_SET_MEMBERSHIP",
                "affected_field": "partial",
        ...
        "correction": {
            "reason": "DATABENTO_SYMBOLOGY_STATUS_FIELDS_ARE_JSON_LISTS",
            "predecessor_partial_validation": "BOOLEAN_SET_MEMBERSHIP_TYPEERROR",
            "successor_partial_validation": "EXACT_EMPTY_STRING_LIST_REQUIRED",
            "successor_not_found_validation": "EXACT_EMPTY_STRING_LIST_REQUIRED",
...
def validate_plan(plan: Mapping[str, object], *, root: Path) -> dict[str, object]:
    ...
    if dict(plan) != expected:
        raise IntegrityError("Apex micro metadata preflight v7 drifted")
...
def load_plan(*, root: Path) -> dict[str, object]:
    return validate_plan(_object(root / PLAN_PATH, "v7 metadata preflight plan"), root=root)
...
def execute_preflight(
    *,
    ...
    """Consume one v7 authorization and run metadata-only operations."""
    ...
    boundary.assert_active_path(
        ...
        purpose="Apex micro v7 metadata report",
        subtree="state/unpublished_evidence/apex_micro_metadata_preflight_v7",
    )
    if report_path.exists():
        raise IntegrityError("Apex micro v7 metadata report is create-only")
