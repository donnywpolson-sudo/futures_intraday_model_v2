"""Gap-proof bounded-partial-safe Apex micro metadata preflight successor.

The executed v14 plan, consumed authorization, and fail-closed report remain
immutable. Databento can retain a bounded opaque ``partial`` status on a
post-effective parent query even after valid mappings begin. V15 never
interprets or records that status content. Instead it requires the exact
request echoes plus a gap-free union of validated instrument-id intervals from
the product-effective start through the end-exclusive bound. HTTP failures,
unrelated roots, gaps, malformed intervals, nonzero cost, and every acquisition
gate remain fail closed. No download-capable API is exposed to this executor.
"""
...
PREDECESSOR_PLAN_ID: Final = (
    "d088d9662f0649ca4b9567499ab09d72e46c576293f959d4d894a6f713d0a5ca"
)
PREDECESSOR_PLAN_SHA256: Final = (
    "fd672f70aae6e7b3ca008821dbbea26f26b1313c3882249cca72cdaad77c619c"
)
...
PREDECESSOR_REPORT_SHA256: Final = (
    "9d29811a5a4f4389aaf51ab47bdb407ded34b1c70fe2f77e7a0de58c14897aaf"
)
PREDECESSOR_AUTHORIZATION_RECEIPT_ID: Final = (
    "5c178067d167ea4dd63fb739321621cd342e88171b67535d73825dee422d6829"
)
...
PREDECESSOR_AUTHORIZATION_SHA256: Final = (
    "6b7e4029fe1e3b8ee454045e0a513ba1d95cb945be5136259557769ebf2f44f6"
)
SCHEMA: Final = "apex_micro_databento_metadata_preflight/15.0.0"
REPORT_SCHEMA: Final = "apex_micro_databento_metadata_preflight_report/15.0.0"
MAXIMUM_STATUS_STRING_LENGTH: Final = 64
MAXIMUM_STATUS_INTEGER_MAGNITUDE: Final = 999_999
MAXIMUM_MESSAGE_STRING_LENGTH: Final = 1_024
MAXIMUM_RESULT_GROUPS: Final = 2_000
MAXIMUM_RESULT_GROUP_KEY_LENGTH: Final = 128
MAXIMUM_RESULT_INTERVALS: Final = 100_000


def load_predecessor_failure(*, root: Path) -> dict[str, object]:
    """Verify exact v14 plan, failure report, and authorization evidence."""
    ...
    if (
        ...
        raise IntegrityError("v14 metadata preflight evidence was not preserved byte-for-byte")
    plan = _object(plan_path, "v14 metadata preflight plan")
    report = _object(report_path, "v14 metadata preflight report")
    plan_core = dict(plan)
    ...
    if (
        ...
        or report.get("predecessor_report_id")
        ...
        or report.get("failed_provider_call_ordinal") != 5
        ...
        or report.get("provider_call_total") != 5
        ...
        raise IntegrityError("v14 metadata preflight failure evidence drifted")
    return report
...
def build_plan(*, root: Path) -> dict[str, object]:
    ...
    predecessor_plan = _object(
        root / PREDECESSOR_PLAN_PATH, "v14 metadata preflight plan"
    )
    ...
    if requests != predecessor_plan.get("requests"):
        raise IntegrityError("v15 market/schema request definitions drifted from v14")
    core: dict[str, object] = {
        "schema_version": SCHEMA,
        "classification": "PREPARED_METADATA_ONLY_PROVIDER_APPROVAL_REQUIRED",
        "state": "PREPARED_NOT_EXECUTED",
        "operation": OPERATION,
        "lane_id": "apex_integer_micro_15",
        "dataset": DATASET,
        ...
        "predecessor_execution": {
            ...
            "provider_call_total": 5,
            "failed_call_inference": {
                "basis": (
                    "SEALED_V14_DISCOVERY_RESULT_GROUPS_PASSED_AND_POST_EFFECTIVE_"
                    "PARENT_PARTIAL_STATUS_REACHED"
                ),
                "call_ordinal": 4,
                "operation": "resolve",
                "market": "MES",
                ...
                "stype_in": "parent",
                "local_defect": (
                    "POST_EFFECTIVE_PARENT_PARTIAL_STATUS_MISINTERPRETED_AS_EMPTY_ONLY"
                ),
                "affected_field_disposition": (
                    "V14_SANITIZED_PARTIAL_FIELD_NAME_ONLY_VALUES_NOT_RECORDED"
                ),
                "sdk_contract_basis": (
                    "MAPPING_INTERVAL_UNION_CAN_PROVE_POST_EFFECTIVE_COVERAGE_"
                    "WITHOUT_INTERPRETING_OPAQUE_PARTIAL_CONTENT"
                ),
        ...
        "correction": {
            "reason": "POST_EFFECTIVE_PARTIAL_REQUIRES_GAP_PROOF_NOT_EMPTY_SEMANTICS",
            "application_status_validation": (
                "BOUNDED_EXACT_INTEGER_OR_NONEMPTY_STRING_SHAPE_ONLY_VALUE_NOT_RECORDED"
            ),
            "maximum_status_string_length": MAXIMUM_STATUS_STRING_LENGTH,
            "maximum_status_integer_magnitude": MAXIMUM_STATUS_INTEGER_MAGNITUDE,
            "success_message_validation": (
                "BOUNDED_EXACT_STRING_SHAPE_ONLY_VALUE_NOT_RECORDED"
            ),
            "maximum_message_string_length": MAXIMUM_MESSAGE_STRING_LENGTH,
            "result_group_validation": (
                "EXACT_REQUEST_ECHO_PLUS_BOUNDED_ROOT_CONSISTENT_GROUP_KEYS_"
                "AND_EXACT_INSTRUMENT_ID_INTERVALS"
            ),
            "maximum_result_groups": MAXIMUM_RESULT_GROUPS,
            "maximum_result_group_key_length": MAXIMUM_RESULT_GROUP_KEY_LENGTH,
            "maximum_result_intervals": MAXIMUM_RESULT_INTERVALS,
            "post_effective_partial_validation": (
                "BOUNDED_OPAQUE_PRESENCE_ONLY_WITH_GAP_FREE_INTERVAL_UNION"
            ),
            "post_effective_interval_coverage": (
                "EXACT_QUERY_START_THROUGH_END_EXCLUSIVE"
            ),
            "http_error_disposition": "DATABENTO_SDK_REJECTS_HTTP_ERRORS_BEFORE_JSON",
            "mapping_success_basis": (
                "EXACT_ECHO_BOUNDED_RESULT_GROUPS_INTERVALS_MESSAGE_PARTIAL_NOT_FOUND_"
                "AND_DATE_GATES"
            ),
            "discovery_partial_content_or_exact_count_recording": "FORBIDDEN",
            "maximum_opaque_partial_entries": MAXIMUM_OPAQUE_PARTIAL_ENTRIES,
            "two_stage_verification": (
                "DISCOVER_FIRST_MAPPING_THEN_REQUERY_PARENT_AND_CONTINUOUS_WITH_"
                "GAP_FREE_INTERVAL_UNION"
            ),
            "dataset_start_effective_date": (
                "FAIL_CLOSED_EXACT_PRODUCT_DATE_UNRESOLVED"
            ),
            "field_specific_failure_diagnostics": (
                "ORDERED_SANITIZED_FIELD_NAME_ONLY_NO_VALUE"
            ),
        ...
        "checks": {
            ...
            "application_status_bounded_opaque_scalar_shape": True,
            "application_status_value_recording_forbidden": True,
            "prelaunch_discovery_bounded_opaque_partial_presence_only": True,
            "prelaunch_discovery_exact_single_symbol_echo": True,
            "bounded_root_consistent_result_groups": True,
            "all_result_group_intervals_validated": True,
            "post_effective_parent_and_continuous_bounded_partial_status": True,
            "post_effective_gap_free_interval_union": True,
            "symbology_success_message_bounded_opaque_string_shape": True,
            "symbology_success_message_value_recording_forbidden": True,
...
def validate_plan(plan: Mapping[str, object], *, root: Path) -> dict[str, object]:
    ...
    if dict(plan) != expected:
        raise IntegrityError("Apex micro metadata preflight v15 drifted")
...
def load_plan(*, root: Path) -> dict[str, object]:
    return validate_plan(_object(root / PLAN_PATH, "v15 metadata preflight plan"), root=root)
...
def _opaque_application_status(value: object) -> str:
    """Validate only the SDK-supported opaque scalar shape, never its value."""

    if type(value) is int:
        if abs(value) > MAXIMUM_STATUS_INTEGER_MAGNITUDE:
            raise IntegrityError("provider symbology application status integer is unbounded")
        return "BOUNDED_INTEGER_VALUE_NOT_RECORDED"
    if type(value) is str:
        if not value or len(value) > MAXIMUM_STATUS_STRING_LENGTH:
            raise IntegrityError("provider symbology application status string is unbounded")
        return "BOUNDED_NONEMPTY_STRING_VALUE_NOT_RECORDED"
    raise IntegrityError("provider symbology application status is not a bounded scalar")


def _opaque_success_message(value: object) -> str:
    """Require a bounded SDK-opaque string without interpreting or recording it."""

    if type(value) is not str or len(value) > MAXIMUM_MESSAGE_STRING_LENGTH:
        raise IntegrityError("provider symbology success message is not a bounded string")
    return (
        "EMPTY_STRING_VALUE_NOT_RECORDED"
        if value == ""
        else "BOUNDED_NONEMPTY_STRING_VALUE_NOT_RECORDED"
    )
    ...
    for group_key, entries in result.items():
        ...
        for entry in entries:
            ...
            try:
                ...
                raise IntegrityError("provider symbology interval date is invalid") from exc
            if d0 < query_start or d0 >= d1 or d1 > end:
            ...
            dates.append(d0)
            coverage_intervals.append((d0, d1))
    first = min(dates)
...
def execute_preflight(
    *,
    ...
    """Consume one v15 authorization and run metadata-only operations."""
    ...
    boundary.assert_active_path(
        ...
        purpose="Apex micro v15 metadata report",
        subtree="state/unpublished_evidence/apex_micro_metadata_preflight_v15",
    )
    if report_path.exists():
        raise IntegrityError("Apex micro v15 metadata report is create-only")
