"""V12 pipeline with true-zero flat-baseline coverage independence."""
...
from .tier1_bracket_v11 import (
    ...
    evaluate_independent_strategies_v11, plan_independent_strategy_v11,
...
def _path_payload(path: v5.AccountPathV5) -> dict[str, object]:
    return {
        ...
        "complete": path.complete, "admitted_trades": len(path.admitted),
...
def run_v12_pipeline(
    *, streams: Mapping[tuple[str, int], Iterable[v5.V5SourceRecord]],
    ...
    if not v5._hex64(trial_id):
        raise IntegrityError("V12 pipeline requires a registered trial identity")
    ...
    coverage_evidence = v5.CoverageEvidence(
        ...
        causal_feature_eligible=sum(
            row.features is not None for row in evaluation_rows
        ),
    ...
    artifacts = v5.EvidenceArtifactsV5(
        ...
        inference={
            ...
            "nested_crossfit_decision_availability": (
                crossfit.controls.decision_availability
            ),
            "nested_crossfit_selected_path_coverage": (
                crossfit.controls.selected_path_coverage
            ),
            "nested_crossfit_evaluation_completeness": (
