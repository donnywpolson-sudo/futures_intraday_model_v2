"""Success-echo-safe Apex micro metadata preflight successor.

The executed v7 plan, consumed authorization, and fail-closed report remain
immutable. V8 preserves v7's exact empty-list requirements for ``partial`` and
``not_found`` while correcting the success-message echo contract. A successful
resolve must have integer status zero and a message in the exact allowlist
``{"", "OK"}``; no other provider content is logged or reported. No
...
PREDECESSOR_PLAN_ID: Final = (
    "4d5c3c86df004643d3958bf8611d66c43d7275c6460b6fe3acb9af3cbc39a723"
)
PREDECESSOR_PLAN_SHA256: Final = (
    "4fb109d59687fa6fd16277c5b87587c69bc96d24f1f43f68ca65a3e7bce2ce91"
)
PREDECESSOR_REPORT_ID: Final = (
    "8a4b701c582420d8c0aaa9ad1eac051ace6c467b4f5f479ef6593eaa5c4a8d6d"
)
PREDECESSOR_REPORT_SHA256: Final = (
    "4addf681b7faa6635b412b0b3c693bc47f2d63e4907a6a26e14ec47309959e41"
)
PREDECESSOR_AUTHORIZATION_RECEIPT_ID: Final = (
    "b6160f88cdf1d2d28bf722088fee64dab99b78d236292f1062241dbd45f8ed4b"
)
...
PREDECESSOR_AUTHORIZATION_SHA256: Final = (
    "d40138b6236fea8494f4cc78bf0eb9a3f4e921eb012b06d50cc317eeccc1d7e2"
)
SCHEMA: Final = "apex_micro_databento_metadata_preflight/8.0.0"
REPORT_SCHEMA: Final = "apex_micro_databento_metadata_preflight_report/8.0.0"
...
def load_predecessor_failure(*, root: Path) -> dict[str, object]:
    """Verify the exact v7 failure and its one-use authorization evidence."""
    ...
    if (
        ...
        raise IntegrityError("v7 metadata preflight evidence was not preserved byte-for-byte")
    plan = _object(plan_path, "v7 metadata preflight plan")
    plan_core = dict(plan)
    plan_id = plan_core.pop("plan_id", None)
    report = _object(report_path, "v7 metadata preflight report")
    ...
    if (
        ...
        != {"market": "MES", "stype_in": "parent", "symbol": "MES.FUT"}
        ...
        raise IntegrityError("v7 metadata preflight failure evidence drifted")
    return report
...
def build_plan(*, root: Path) -> dict[str, object]:
    ...
    predecessor_plan = _object(
        root / PREDECESSOR_PLAN_PATH, "v7 metadata preflight plan"
    )
    ...
    if requests != predecessor_plan.get("requests"):
        raise IntegrityError("v8 market/schema request definitions drifted from v7")
    core: dict[str, object] = {
        ...
        "predecessor_execution": {
            ...
            "failed_call_inference": {
                "basis": (
                    "SEALED_V7_INTEGRITY_ERROR_IN_PRE_LIST_EXACT_ECHO_TUPLE_PLUS_"
                    "INSTALLED_SDK_TEN_FIELD_RESPONSE_CONTRACT"
                ),
                "call_ordinal": 4,
                "operation": "resolve",
                "market": "MES",
                "symbol": "MES.FUT",
                "stype_in": "parent",
                "candidate_local_defect": "OVERSTRICT_EMPTY_SUCCESS_MESSAGE_EXPECTATION",
                "candidate_affected_field": "message",
                "inference_state": "LOCAL_CONTRACT_REMEDIATION_NOT_PROVIDER_FACT",
        ...
        "correction": {
            "reason": "DATABENTO_SUCCESS_MESSAGE_ECHO_REQUIRES_BOUNDED_ALLOWLIST",
            "predecessor_success_message_validation": "EXACT_EMPTY_STRING_ONLY",
            "successor_success_message_validation": "EXACT_EMPTY_OR_OK_STRING_WITH_INTEGER_STATUS_ZERO",
            "partial_validation": "EXACT_EMPTY_STRING_LIST_REQUIRED",
            "not_found_validation": "EXACT_EMPTY_STRING_LIST_REQUIRED",
            "nonempty_partial_or_not_found": "FAIL_CLOSED_NO_SUBSTITUTION",
            "field_specific_failure_diagnostics": "SANITIZED_FIELD_NAME_ONLY_NO_VALUE",
            ...
            "scope_change": (
                "LOCAL_RESPONSE_VALIDATION_CORRECTION_ONLY_NO_MARKET_SCHEMA_OR_DATA_ENDPOINT"
            ),
        },
        "obsolete_plan": {
        ...
        "checks": {
            ...
            "symbology_status_exact_empty_list_shape": True,
...
def validate_plan(plan: Mapping[str, object], *, root: Path) -> dict[str, object]:
    ...
    if dict(plan) != expected:
        raise IntegrityError("Apex micro metadata preflight v8 drifted")
...
def load_plan(*, root: Path) -> dict[str, object]:
    return validate_plan(_object(root / PLAN_PATH, "v8 metadata preflight plan"), root=root)
...
def _empty_symbol_status(value: object, *, name: str) -> int:
...
def _symbology_summary(
    ...
    *,
    ...
    partial_count = _empty_symbol_status(value.get("partial"), name="partial")
    not_found_count = _empty_symbol_status(value.get("not_found"), name="not_found")
...
def execute_preflight(
    *,
    ...
    """Consume one v8 authorization and run metadata-only operations."""
    ...
    boundary.assert_active_path(
        ...
        purpose="Apex micro v8 metadata report",
        subtree="state/unpublished_evidence/apex_micro_metadata_preflight_v8",
    )
    if report_path.exists():
        raise IntegrityError("Apex micro v8 metadata report is create-only")
        ...
        for market in CURRENT_ACQUISITION_MARKETS:
            ...
            for stype_in, symbol in (
                ...
                raw = budget.call(
                    "resolve",
                    apis.resolve,
                    context={"market": market, "stype_in": stype_in, "symbol": symbol},
                    dataset=DATASET,
                    symbols=[symbol],
                    stype_in=stype_in,
                    stype_out="instrument_id",
