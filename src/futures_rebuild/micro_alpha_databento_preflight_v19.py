"""Opaque-partial-semantic-safe Apex micro metadata preflight successor.

The executed v18 plan, consumed authorization, and fail-closed report remain
immutable. V18 passed MES, MCL, and MGC symbology verification, then rejected
M6E discovery because it still interpreted opaque ``partial`` presence as a
prelaunch signal. V19 keeps ``partial`` bounded and shape-checked but derives
the product-effective date only from the first validated mapping interval.
Parent-family boundary coverage, continuous-roll gap proof, exact echoes,
identities, costs, disk, and destination gates remain. No raw interval values,
DBN rows, or download-capable API are exposed.
"""
...
from .micro_alpha_databento_preflight_v18 import (
    PLAN_PATH as PREDECESSOR_PLAN_PATH,
    REPORT_PATH as PREDECESSOR_REPORT_PATH,
    load_plan as load_v18_plan,
)
...
PREDECESSOR_PLAN_ID: Final = (
    "4f85ba9f26f846173d490637ba8c3c70d5deffefacd19222d1d75ebdd0ef847e"
)
PREDECESSOR_PLAN_SHA256: Final = (
    "9a04d75cb60b1b0e0aa31b89548700be09602ecf39026c4b859e2f0929010bb0"
)
PREDECESSOR_REPORT_ID: Final = (
    "2d267aa18e7d5b027620ffe4c088de536c6f814420281426c6a004b3b00cd9cf"
)
PREDECESSOR_REPORT_SHA256: Final = (
    "6f4169447eb951c48657c3072d8974fee859a6c1aac7920e53e394808709f85b"
)
PREDECESSOR_AUTHORIZATION_RECEIPT_ID: Final = (
    "380f9d216534a6ba7aa97d267e9fb96e86f4f84f7f898d82366b6a5007992219"
)
...
PREDECESSOR_AUTHORIZATION_SHA256: Final = (
    "cfe5a7b83de25fcf1bb5013496fd21dc031fd5fc751294e7c8e476fe87e0067f"
)
SCHEMA: Final = "apex_micro_databento_metadata_preflight/19.0.0"
REPORT_SCHEMA: Final = "apex_micro_databento_metadata_preflight_report/19.0.0"


def load_predecessor_failure(*, root: Path) -> dict[str, object]:
    """Verify exact v18 plan, failure report, and authorization evidence."""

    plan = load_v18_plan(root=root)
    ...
    if (
        ...
        raise IntegrityError("v18 predecessor evidence drifted")
    report = _object(report_path, "v18 metadata preflight failure report")
    expected_context = {
        "market": "M6E",
        "stage": "discovery",
        "stype_in": "parent",
    ...
    if (
        ...
        or report.get("failure_code") != "PRELAUNCH_PARTIAL_DISPOSITION_DRIFT"
        or report.get("exception_type") != "IntegrityError"
        or report.get("failed_provider_call_ordinal") != 13
        ...
        or report.get("provider_call_total") != 13
        ...
        raise IntegrityError("v18 predecessor failure classification drifted")
    return report


def build_plan(*, root: Path) -> dict[str, object]:
    failure = load_predecessor_failure(root=root)
    predecessor = load_v18_plan(root=root)
    ...
    core.update(
        {
            ...
            "predecessor_execution": {
                ...
                "failure_code": "PRELAUNCH_PARTIAL_DISPOSITION_DRIFT",
                "exception_type": "IntegrityError",
                "provider_call_total": 13,
                "failed_provider_operation": "resolve",
                "failed_provider_call_ordinal": 13,
                "failed_request_context": failure["failed_request_context"],
                "failed_validation_field": "partial",
                "automatic_retries": 0,
                "preservation": "IMMUTABLE_NO_OVERWRITE_DELETE_OR_RELABEL",
            },
            "correction": {
                **dict(predecessor["correction"]),
                "reason": "V18_M6E_DISCOVERY_OPAQUE_PARTIAL_PRESENCE_INTERPRETED",
                "product_effective_date_basis": (
                    "FIRST_VALIDATED_PARENT_MAPPING_INTERVAL_ONLY"
                ),
                "discovery_partial_semantics_interpreted": False,
                "partial_presence_required_for_prelaunch_date": False,
                "partial_field_validation": (
                    "BOUNDED_OPAQUE_STRING_LIST_SHAPE_ONLY_VALUES_NOT_RECORDED"
                ),
    ...
    checks.update(
        {
            "product_effective_date_from_first_validated_mapping_interval": True,
            "opaque_partial_presence_semantics_forbidden": True,
            "opaque_partial_values_recording_forbidden": True,
        }
    )
    core["checks"] = checks
...
def validate_plan(plan: Mapping[str, object], *, root: Path) -> dict[str, object]:
    ...
    if dict(plan) != expected:
        raise IntegrityError("Apex micro metadata preflight v19 plan drifted")
...
def load_plan(*, root: Path) -> dict[str, object]:
    return validate_plan(_object(root / PLAN_PATH, "v19 metadata preflight plan"), root=root)
...
def _symbology_summary(
    ...
    *,
    ...
    disposition = (
        ...
        else "PROVIDER_PRELAUNCH_FIRST_VALIDATED_MAPPING_DATE"
    ...
    return {
        ...
        "partial_semantics_interpreted": False,
        "partial_content_or_exact_count_recorded": False,
        "product_effective_date_basis": (
            "FIRST_VALIDATED_PARENT_MAPPING_INTERVAL_ONLY"
            if allow_discovery_partial
            else "BOUND_POST_EFFECTIVE_QUERY"
        ),
...
def execute_preflight(
    *,
    ...
    """Consume one v19 authorization and run metadata-only operations."""
    ...
    boundary.assert_active_path(
        ...
        purpose="Apex micro v19 metadata report",
        subtree="state/unpublished_evidence/apex_micro_metadata_preflight_v19",
    )
    if report_path.exists():
        raise IntegrityError("Apex micro v19 metadata report is create-only")
