"""SDK-contract-safe Apex micro metadata preflight successor.

The executed v11 plan, consumed authorization, and fail-closed report remain
immutable. Databento's installed SDK requires the symbology ``status`` key but
does not interpret its value when constructing an instrument map. V12 therefore
accepts only a bounded scalar shape for that opaque application-status field and
records only the shape classification. HTTP failures remain rejected by the SDK,
while exact request echoes, result keys, mapping intervals, prelaunch handling,
and every acquisition gate remain fail closed. No download-capable API is
...
PREDECESSOR_PLAN_ID: Final = (
    "ef1a3156f7c62ba5f5ebddfd86a27ae60bb27eab9bb87e4afec6439474f964bf"
)
PREDECESSOR_PLAN_SHA256: Final = (
    "5f5c440f4581463dd11ec25224c7adfa7c81027147bdc98c64896163bf9d4015"
)
...
PREDECESSOR_REPORT_SHA256: Final = (
    "cf08e800148d66f552186f3c9eb20ad8bd4110a618a04874711609fd15ae8e6c"
)
PREDECESSOR_AUTHORIZATION_RECEIPT_ID: Final = (
    "b09db582730cf9d724928ed184a0d59f36d1598c46a35cf2bcff032000a04d9d"
)
...
PREDECESSOR_AUTHORIZATION_SHA256: Final = (
    "d22f7ced224fa1912890396092ae5dc85eb39200eca6aa3260d2090f8a6f95e1"
)
SCHEMA: Final = "apex_micro_databento_metadata_preflight/12.0.0"
REPORT_SCHEMA: Final = "apex_micro_databento_metadata_preflight_report/12.0.0"
...
def load_predecessor_failure(*, root: Path) -> dict[str, object]:
    """Verify exact v11 plan, failure report, and authorization evidence."""
    ...
    if (
        ...
        raise IntegrityError("v11 metadata preflight evidence was not preserved byte-for-byte")
    plan = _object(plan_path, "v11 metadata preflight plan")
    report = _object(report_path, "v11 metadata preflight report")
    plan_core = dict(plan)
    ...
    if (
        ...
        != "50955816f1bbcb2a1a06c820ea585dd0f16bf02b34651dfc3b194a1cc28d2e63"
        or report.get("state") != "FAIL_CLOSED_METADATA_ONLY"
        or report.get("failure_code") != "SYMBOL_ECHO_FIELD_DRIFT"
        ...
        or report.get("failed_validation_field") != "status"
        ...
        raise IntegrityError("v11 metadata preflight failure evidence drifted")
    return report
...
def build_plan(*, root: Path) -> dict[str, object]:
    ...
    predecessor_plan = _object(
        root / PREDECESSOR_PLAN_PATH, "v11 metadata preflight plan"
    )
    ...
    if requests != predecessor_plan.get("requests"):
        raise IntegrityError("v12 market/schema request definitions drifted from v11")
    core: dict[str, object] = {
        "schema_version": SCHEMA,
        "classification": "PREPARED_METADATA_ONLY_PROVIDER_APPROVAL_REQUIRED",
        "state": "PREPARED_NOT_EXECUTED",
        "operation": OPERATION,
        "lane_id": "apex_integer_micro_12",
        "dataset": DATASET,
        ...
        "predecessor_execution": {
            ...
            "failed_call_inference": {
                "basis": (
                    "SEALED_V11_EXACT_REQUEST_AND_RESPONSE_SHAPE_REACHED_"
                ...
                "stype_in": "parent",
                "local_defect": (
                    "SDK_OPAQUE_APPLICATION_STATUS_MISINTERPRETED_AS_EXACT_ZERO"
                ),
                "affected_field_disposition": (
                    "V11_SANITIZED_STATUS_FIELD_NAME_ONLY_VALUE_NOT_RECORDED"
                ),
                "sdk_contract_basis": (
                    "INSTALLED_DATABENTO_INSTRUMENT_MAP_REQUIRES_STATUS_KEY_"
        ...
        "correction": {
            "reason": "SDK_OPAQUE_APPLICATION_STATUS_REQUIRES_SHAPE_NOT_VALUE_GATE",
            "application_status_validation": (
...
def validate_plan(plan: Mapping[str, object], *, root: Path) -> dict[str, object]:
    ...
    if dict(plan) != expected:
        raise IntegrityError("Apex micro metadata preflight v12 drifted")
...
def load_plan(*, root: Path) -> dict[str, object]:
    return validate_plan(_object(root / PLAN_PATH, "v12 metadata preflight plan"), root=root)
...
def execute_preflight(
    *,
    ...
    """Consume one v12 authorization and run metadata-only operations."""
    ...
    boundary.assert_active_path(
        ...
        purpose="Apex micro v12 metadata report",
        subtree="state/unpublished_evidence/apex_micro_metadata_preflight_v12",
    )
    if report_path.exists():
        raise IntegrityError("Apex micro v12 metadata report is create-only")
