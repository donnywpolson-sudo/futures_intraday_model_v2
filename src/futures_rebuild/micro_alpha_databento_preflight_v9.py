"""Prelaunch-discovery-safe Apex micro metadata preflight successor.

The executed v8 plan, consumed authorization, and fail-closed report remain
immutable. V9 permits exactly one discovery-only ``partial`` entry equal to
the requested parent symbol, derives its first mapping date, then re-resolves
both parent and continuous symbology from that date with empty ``partial`` and
``not_found`` lists required. Dataset-start ambiguity and every later gap still
fail closed. No download-capable API is exposed to this executor.
"""
...
PREDECESSOR_PLAN_ID: Final = (
    "112f1f792f849d49718a2ae19ff27bb60395a3d4027b5d82e0b322d4d9f5d1d8"
)
PREDECESSOR_PLAN_SHA256: Final = (
    "a84b5aae2561957ba7d51be42b52d47f5b0bf18e8ec9438b55a348a29f16751c"
)
PREDECESSOR_REPORT_ID: Final = (
    "358be3ed7f3c9089fe045c07e1cb05c675c40a194be5ca654fbfe08422839964"
)
PREDECESSOR_REPORT_SHA256: Final = (
    "e650fa4a00c9ae80100c9c9719c2a59d270bafd824b51a56c50a900c9759ccf6"
)
PREDECESSOR_AUTHORIZATION_RECEIPT_ID: Final = (
    "53ef328db8b436ef91987b2acdfb6446d59b75356515c4ea7aa7c6085ff412b7"
)
...
PREDECESSOR_AUTHORIZATION_SHA256: Final = (
    "fc2acaed2e7c08bf04aab5afb4452dbb96ff9244e53278f92a21aad8412d76d0"
)
SCHEMA: Final = "apex_micro_databento_metadata_preflight/9.0.0"
REPORT_SCHEMA: Final = "apex_micro_databento_metadata_preflight_report/9.0.0"
...
def load_predecessor_failure(*, root: Path) -> dict[str, object]:
    """Verify the exact v8 failure and its one-use authorization evidence."""
    ...
    if (
        ...
        raise IntegrityError("v8 metadata preflight evidence was not preserved byte-for-byte")
    plan = _object(plan_path, "v8 metadata preflight plan")
    plan_core = dict(plan)
    plan_id = plan_core.pop("plan_id", None)
    report = _object(report_path, "v8 metadata preflight report")
    ...
    if (
        ...
        or report.get("failed_validation_field") != "symbols"
        ...
        raise IntegrityError("v8 metadata preflight failure evidence drifted")
    return report
...
def build_plan(*, root: Path) -> dict[str, object]:
    ...
    predecessor_plan = _object(
        root / PREDECESSOR_PLAN_PATH, "v8 metadata preflight plan"
    )
    ...
    if requests != predecessor_plan.get("requests"):
        raise IntegrityError("v9 market/schema request definitions drifted from v8")
    core: dict[str, object] = {
        ...
        "predecessor_execution": {
            ...
            "failed_call_inference": {
                "basis": (
                    "SEALED_V8_NONEMPTY_SYMBOLOGY_STATUS_AT_BROAD_PRELAUNCH_QUERY_"
                    "PLUS_V6_LIST_SHAPE_EVIDENCE_AND_PROVIDER_RESULT_MAPPING"
                ),
                "call_ordinal": 4,
                "operation": "resolve",
                "market": "MES",
                "symbol": "MES.FUT",
                "stype_in": "parent",
                "local_defect": "PRELAUNCH_PARTIAL_TREATED_AS_UNCONDITIONAL_FAILURE",
                "affected_field_disposition": "V8_SANITIZED_CLASSIFIER_AMBIGUOUS_SYMBOLS_LABEL",
                "inference_state": "PRELAUNCH_HYPOTHESIS_REQUIRES_TWO_STAGE_PROVIDER_PROOF",
        ...
        "correction": {
            "reason": "PRELAUNCH_DISCOVERY_REQUIRES_TWO_STAGE_SYMBOLOGY_PROOF",
            "discovery_parent_partial_validation": "EMPTY_OR_EXACT_SINGLE_REQUESTED_SYMBOL",
            ...
            "scope_change": (
                "FOUR_ADDITIONAL_SYMBOLOGY_CALLS_ONLY_NO_MARKET_SCHEMA_OR_DATA_ENDPOINT"
            ),
        },
        "obsolete_plan": {
        ...
        "checks": {
            ...
            "prelaunch_discovery_single_symbol_partial_only": True,
...
def validate_plan(plan: Mapping[str, object], *, root: Path) -> dict[str, object]:
    ...
    if dict(plan) != expected:
        raise IntegrityError("Apex micro metadata preflight v9 drifted")
...
def load_plan(*, root: Path) -> dict[str, object]:
    return validate_plan(_object(root / PLAN_PATH, "v9 metadata preflight plan"), root=root)
...
def _symbol_status(
    ...
    *,
    ...
    if not value:
        return 0
    if name == "partial" and allow_single_symbol_partial and value == [symbol]:
        return 1
...
def execute_preflight(
    *,
    ...
    """Consume one v9 authorization and run metadata-only operations."""
    ...
    boundary.assert_active_path(
        ...
        purpose="Apex micro v9 metadata report",
        subtree="state/unpublished_evidence/apex_micro_metadata_preflight_v9",
    )
    if report_path.exists():
        raise IntegrityError("Apex micro v9 metadata report is create-only")
