"""Final additive lifecycle with identical pre/post activation certification."""
...
from .tier1_authoritative_certified_execution import (
    INVALID_DISPOSITION,
...
INVALID_PREPARATION_PATH = Path(
    "configs/tier1_authoritative_692a_invalid_retirement_preparation.json"
)
SYNTHETIC_VERIFICATION_PATH = Path(
    "configs/tier1_authoritative_certified_synthetic_verification.json"
)
PREPARED_CERTIFICATE_PATH = Path(
    "configs/tier1_authoritative_certified_preexecution_certificate.json"
)
INVALID_RETIREMENT_EVENT_ROOT = Path(
    "state/trial_events/tier1_authoritative_postactivation_invalid_retirement"
)
TRIAL_EVENT_ROOT = Path("state/trial_events/tier1_authoritative_trial")
PUBLISHED_REGISTRY_PATH = (
    REGISTRY_ROOT / f"{INVALID_TRIAL_ID}.json"
)
...
def _object(path: Path) -> dict[str, object]:
    try:
        ...
        raise IntegrityError(f"invalid certified lifecycle artifact: {path}") from exc
    if not isinstance(value, dict):
        raise IntegrityError("certified lifecycle artifact is not an object")
    return value


def load_692a_invalid_retirement_preparation(*, root: Path) -> dict[str, object]:
    ...
    if (
        ...
        != "tier1_authoritative_postactivation_invalid_retirement/1.0.0"
        ...
        raise IntegrityError("692a invalid-retirement preparation drifted")
    return preparation
...
def load_certified_synthetic_verification(*, root: Path) -> dict[str, object]:
    ...
    if (
        ...
        != "tier1_authoritative_certified_synthetic_verification/1.0.0"
        ...
        or any(len(value) < 9 for value in replacement.values() if isinstance(value, list))
        or any(not isinstance(value, list) for value in replacement.values())
        or any(
            ...
            for field in (
                ...
                "prepublication_context_tested",
                "postpublication_context_tested",
                "rollback_context_tested",
        ...
        raise IntegrityError("certified synthetic verification is incomplete or drifted")
    return verification
...
def _bindings(root: Path) -> dict[str, str]:
    ...
    if not isinstance(inherited, Mapping) or not inherited:
        raise IntegrityError("published 692a registry bindings are unavailable")
    ...
    additions = (
        ...
        Path("src/futures_rebuild/tier1_authoritative_certified_execution.py"),
        Path("src/futures_rebuild/tier1_authoritative_certified_lifecycle.py"),
        Path("scripts/run_tier1_authoritative_certified_historical_execution.py"),
        Path("scripts/publish_tier1_authoritative_certified_lifecycle.py"),
        Path("tests/test_tier1_authoritative_certified_lifecycle.py"),
    )
    bindings.update({path.as_posix(): sha256_file(root / path) for path in additions})
    if "configs/active_tier1_trial.json" in bindings:
        raise IntegrityError("mutable active pointer entered certified bindings")
    if any(sha256_file(root / path) != digest for path, digest in bindings.items()):
        raise IntegrityError("inherited or additive certified binding drifted")
    return dict(sorted(bindings.items()))


def build_certified_lifecycle_payloads(
    *,
    ...
    if (
        ...
        raise IntegrityError("certified lifecycle inputs are incomplete")
...
def prepare_certified_authoritative_lifecycle(
    *, root: Path
) -> PreparedAuthoritativeLifecycle:
    ...
    if (
        ...
        raise IntegrityError("certified prepublication pointer condition failed")
    prepared = build_certified_lifecycle_payloads(
        protocol=protocol,
...
def transition_documents(
    *, root: Path
    ...
    """Select lifecycle state from the pointer identity, never registry presence."""
    ...
    trial_id, active, registry, certificate = load_certified_registered_context(
        root=root, plan=plan
    )
    ...
    if trial_id != registry.get("trial_id"):
        raise IntegrityError("certified transition identity is inconsistent")
    return plan, {
        "pointer": active,
        "registry": registry,
...
def load_prepared_certified_certificate(*, root: Path) -> dict[str, object]:
    prepared = prepare_certified_authoritative_lifecycle(root=root)
    ...
    if certificate != expected:
        raise IntegrityError("prepared certified authoritative certificate drifted")
    return certificate


def persist_certified_authoritative_lifecycle(
    *,
    root: Path,
    prepared: PreparedAuthoritativeLifecycle,
    post_activation_check: Callable[[], object],
) -> dict[str, str]:
    """Publish create-only records, activate last, and rollback on any postcheck."""

    if prepared != prepare_certified_authoritative_lifecycle(root=root):
        raise IntegrityError("certified lifecycle changed after preparation")
    load_prepared_certified_certificate(root=root)
    documents = published_documents(prepared)
    plan = load_authoritative_execution_plan(root=root)
    validate_certified_registered_context_documents(
        plan=plan,
    ...
    retirement_path = INVALID_RETIREMENT_ROOT / f"{prepared.failed_retirement_id}.json"
    retirement_event = INVALID_RETIREMENT_EVENT_ROOT / f"{prepared.failed_retirement_id}.json"
    ...
    payloads = (
        ...
        (
            retirement_event,
            {
                "schema_version": "tier1_authoritative_postactivation_invalid_retirement_event/1.0.0",
    ...
    if any((root / path).exists() for path, _ in payloads):
        raise IntegrityError("certified publication destination already exists")
    if sha256_file(root / ACTIVE_POINTER_PATH) != EXPECTED_PREPUBLICATION_POINTER_SHA256:
        raise IntegrityError("active pointer changed before certified publication")
    ...
    def validate_after_activation() -> None:
        load_certified_registered_context(root=root, plan=plan)
