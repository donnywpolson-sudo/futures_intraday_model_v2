...
V21 remains immutable failure evidence.  V23 downloads every authorized annual
...
OPERATION: Final = "ACQUIRE_APEX_MICRO_TIER01_RAW_DBN_INACTIVE_CUSTODY_V23_ONCE"
STAGING_ROOT: Final = Path(
    "state/provider_acquisition_staging/apex_micro_tier01_v23"
)
...
def _validate_predecessor_evidence(
    *, root: Path
    ...
    return plan, failure, supersession
...
def build_acquisition_plan(
    *, root: Path, committed_head: str, require_destination_absence: bool = True
    ...
    if committed_head != _git_head(root):
        raise IntegrityError("v23 plan must bind the live committed HEAD")
    predecessor, failure, v22_supersession = _validate_predecessor_evidence(
        root=root
    )
    requests = json.loads(json.dumps(predecessor["requests"]))
    if not isinstance(requests, list) or len(requests) != MAXIMUM_DBN_FILES:
        raise IntegrityError("v21 request set is invalid for v23")
    destinations = [
        str(item[key])
    ...
    if len(set(destinations)) != 320:
        raise IntegrityError("v23 destinations collide")
    if require_destination_absence and any((root / path).exists() for path in destinations):
        raise IntegrityError("v23 destination already exists")
...
def load_acquisition_plan(
    *, root: Path, require_destination_absence: bool = True
    ...
    root = root.resolve(strict=True)
    plan = _object(root / PLAN_PATH, "v23 acquisition plan")
    if (
        ...
        raise UnauthorizedOperation("v23 acquisition plan is absent or drifted")
    ...
    if plan != expected:
        raise IntegrityError("v23 acquisition plan does not reconstruct exactly")
    ...
    if (
        ...
        raise IntegrityError("v23 acquisition limits drifted")
    return plan
    ...
    *,
    ...
    try:
        for item in items:
            ...
            if remaining <= 0:
                raise UnauthorizedOperation("v23 acquisition runtime ceiling reached")
            request_id = str(item["request_id"])
            failed_request_id = request_id
            partial = downloads / f"{request_id[:16]}.dbn.zst.partial"
            if partial.exists():
                raise IntegrityError("v23 staging destination already exists")
            _set_timeout(
                apis.get_range,
            ...
            if not partial.is_file():
                raise IntegrityError("provider did not create the v23 staging file")
            size = partial.stat().st_size
            if size <= 0 or size > item["request_byte_ceiling"]:
                raise UnauthorizedOperation("v23 file is empty or exceeds its ceiling")
            digest = sha256_file(partial)
            with total_lock:
                proposed = total_state["bytes"] + size
                if proposed > maximum_total_bytes:
                    raise UnauthorizedOperation("v23 total byte ceiling exceeded")
                total_state["bytes"] = proposed
            sidecar_path = downloads / f"{request_id[:16]}.manifest.json.partial"
            sidecar_core = {
                "schema_version": "apex_micro_inactive_dbn_manifest/23.0.0",
...
def execute_authorized_acquisition(
    *,
    ...
    """Execute v23 once without reusing any v21 staging bytes."""

    root = root.resolve(strict=True)
    boundary = RepoBoundary(root)
    plan = load_acquisition_plan(root=root)
    if credential_source != CREDENTIAL_SOURCE:
        raise UnauthorizedOperation("v23 credential source is not bound")
    ...
    if any(path.exists() for path in destinations):
        raise IntegrityError("v23 create-only destination already exists")
    free = getattr(disk_usage(root), "free", None)
    if type(free) is not int or free < plan["limits"]["required_free_disk_bytes"]:
        raise UnauthorizedOperation("insufficient disk capacity for v23")
    ...
    boundary.assert_active_path(
        attempt.absolute(),
        purpose="Apex micro v23 acquisition staging",
    ...
    base: dict[str, object] = {
        "schema_version": "apex_micro_phase1a_acquisition_terminal/23.0.0",
    ...
    try:
        ...
        for item in plan["requests"]:
            remaining = MAXIMUM_RUNTIME_SECONDS - (clock() - started)
            if remaining <= 0:
                raise UnauthorizedOperation("v23 runtime exhausted before download")
        ...
        with _ConcurrentWarningCollector() as warning_collector:
            with ThreadPoolExecutor(
                ...
                thread_name_prefix="apex-micro-v23-dbn",
            ) as executor:
                futures = [
                    executor.submit(
                        _download_worker,
        ...
        if worker_failures:
            raise IntegrityError("v23 bounded download worker failed")
        if (
            provider_calls != {"get_cost": exact_count, "get_range": exact_count}
            or len(staged) != exact_count
            or provider_client_count != 3
        ):
            raise IntegrityError("v23 successful call or staging count drifted")
        failure_stage = "FINAL_DESTINATION_RECHECK"
        if any(path.exists() for path in destinations):
            raise IntegrityError("v23 destination appeared before finalization")
        ...
        for item in staged:
            ...
            if (
                ...
                raise IntegrityError("v23 finalized DBN hash differs")
            failure_substage = "REVERIFY_FINAL_SIDECAR"
            sidecar = _object(final_sidecar, "v23 final sidecar")
            if not _self_hashed(sidecar, "manifest_id"):
                raise IntegrityError("v23 finalized sidecar identity differs")
...
def verify_completed_acquisition(
    *, root: Path, terminal_path: Path
    ...
    terminal = _object(terminal_abs, "v23 acquisition terminal")
    ...
    if (
        ...
        raise IntegrityError("v23 terminal safety or count drifted")
    records = terminal.get("accepted_files")
    if not isinstance(records, list) or len(records) != exact_count:
        raise IntegrityError("v23 accepted-file census drifted")
    total = 0
    for record in records:
        ...
        if (
            ...
            raise IntegrityError("v23 accepted DBN differs")
        sidecar = _object(sidecar_path, "v23 accepted sidecar")
        if (
            ...
            raise IntegrityError("v23 accepted sidecar differs")
    ...
    if (
        ...
        raise IntegrityError("v23 inactive custody reconciliation is incomplete")
    return {
...
def build_plan_audit(
    *,
    ...
    cleanup = _object(root / CLEANUP_CENSUS_PATH, "v8 cleanup census")
    ...
    if dict(fresh_cleanup_census) != cleanup:
        raise IntegrityError("fresh v8 cleanup census drifted")
    if (
        ...
        raise IntegrityError("v23 topology or cleanup evidence drifted")
    free = getattr(disk_usage(root), "free", None)
    required = plan["limits"]["required_free_disk_bytes"]
    if type(free) is not int or free < required:
        raise UnauthorizedOperation("insufficient disk for v23 plan")
    destinations = [
        str(item[key])
        for item in plan["requests"]
        for key in ("dbn_destination", "sidecar_destination")
    ...
    if any((root / path).exists() for path in destinations):
        raise UnauthorizedOperation("v23 destination conflict appeared")
    core: dict[str, object] = {
        "schema_version": "apex_micro_phase1a_acquisition_plan_audit/23.0.0",
