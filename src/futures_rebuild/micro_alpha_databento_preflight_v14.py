"""Provider-result-group-safe Apex micro metadata preflight successor.

The executed v13 plan, consumed authorization, and fail-closed report remain
immutable. Databento's installed SDK iterates every bounded ``result`` mapping
group and does not require the result keys to equal the single request echo.
V14 therefore binds identity to the exact request echoes, validates every
root-consistent result group and every instrument-id interval, and records only
bounded shape summaries and hashes. HTTP failures, unrelated roots, malformed
intervals, nonzero cost, and every acquisition-gate failure remain fail closed.
...
PREDECESSOR_PLAN_ID: Final = (
    "1efb11e02300257e7ac8c69b218484d820b7b8e412ba109a8292f35a7e59ded8"
)
PREDECESSOR_PLAN_SHA256: Final = (
    "5e02d8184d8de655934fcedae8acbee2b356f55c289e688f1bd8dac9429e86b9"
)
...
PREDECESSOR_REPORT_SHA256: Final = (
    "f7bfa150dbef997160b94b38097cb9582cbd3f3f3e1a12d5be91e0d5169cae3e"
)
PREDECESSOR_AUTHORIZATION_RECEIPT_ID: Final = (
    "ac33198982539e8b895cd27edfc72cb57545b96c39b4225dc579d748897c7a0c"
)
...
PREDECESSOR_AUTHORIZATION_SHA256: Final = (
    "8b97dfc16b71565348f5a9c63f8f9628b9073e19f0fd45117f188318a48c957c"
)
SCHEMA: Final = "apex_micro_databento_metadata_preflight/14.0.0"
REPORT_SCHEMA: Final = "apex_micro_databento_metadata_preflight_report/14.0.0"
...
def load_predecessor_failure(*, root: Path) -> dict[str, object]:
    """Verify exact v13 plan, failure report, and authorization evidence."""
    ...
    if (
        ...
        raise IntegrityError("v13 metadata preflight evidence was not preserved byte-for-byte")
    plan = _object(plan_path, "v13 metadata preflight plan")
    report = _object(report_path, "v13 metadata preflight report")
    plan_core = dict(plan)
    ...
    if (
        ...
        != "6ace02b2d47c197a30578e37929935bd74f6256f796640065178f4a5bcaba5bd"
        ...
        raise IntegrityError("v13 metadata preflight failure evidence drifted")
    return report
...
def build_plan(*, root: Path) -> dict[str, object]:
    ...
    predecessor_plan = _object(
        root / PREDECESSOR_PLAN_PATH, "v13 metadata preflight plan"
    )
    ...
    if requests != predecessor_plan.get("requests"):
        raise IntegrityError("v14 market/schema request definitions drifted from v13")
    core: dict[str, object] = {
        "schema_version": SCHEMA,
        "classification": "PREPARED_METADATA_ONLY_PROVIDER_APPROVAL_REQUIRED",
        "state": "PREPARED_NOT_EXECUTED",
        "operation": OPERATION,
        "lane_id": "apex_integer_micro_14",
        "dataset": DATASET,
        ...
        "predecessor_execution": {
            ...
            "failed_call_inference": {
                "basis": (
                    "SEALED_V13_EXACT_REQUEST_ECHO_AND_RESPONSE_SHAPE_REACHED_"
                ...
                "stype_in": "parent",
                "local_defect": (
                    "SDK_RESULT_GROUPS_MISINTERPRETED_AS_EXACT_SINGLE_REQUEST_KEY"
                ),
                "affected_field_disposition": (
                    "V13_SANITIZED_RESULT_FIELD_NAME_ONLY_VALUES_NOT_RECORDED"
                ),
                "sdk_contract_basis": (
                    "INSTALLED_DATABENTO_INSTRUMENT_MAP_ITERATES_ALL_RESULT_GROUPS_"
                    "WITHOUT_REQUIRING_KEYS_TO_EQUAL_REQUEST_ECHO"
                ),
        ...
        "correction": {
            "reason": "SDK_RESULT_GROUP_KEYS_REQUIRE_BOUNDED_SEMANTIC_VALIDATION",
            ...
            "two_stage_verification": (
                "DISCOVER_FIRST_MAPPING_THEN_REQUERY_PARENT_AND_CONTINUOUS"
            ),
        ...
        "checks": {
            ...
            "post_effective_parent_and_continuous_empty_status_lists": True,
...
def validate_plan(plan: Mapping[str, object], *, root: Path) -> dict[str, object]:
    ...
    if dict(plan) != expected:
        raise IntegrityError("Apex micro metadata preflight v14 drifted")
...
def load_plan(*, root: Path) -> dict[str, object]:
    return validate_plan(_object(root / PLAN_PATH, "v14 metadata preflight plan"), root=root)
...
def _symbology_summary(
    ...
    *,
    ...
    partial_present = _symbol_status(
        value.get("partial"),
        name="partial",
        allow_discovery_partial=allow_discovery_partial,
    )
...
def execute_preflight(
    *,
    ...
    """Consume one v14 authorization and run metadata-only operations."""
    ...
    boundary.assert_active_path(
        ...
        purpose="Apex micro v14 metadata report",
        subtree="state/unpublished_evidence/apex_micro_metadata_preflight_v14",
    )
    if report_path.exists():
        raise IntegrityError("Apex micro v14 metadata report is create-only")
