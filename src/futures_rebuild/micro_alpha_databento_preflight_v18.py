"""Parent-family-aware Apex micro metadata preflight successor.

The executed v17 plan, consumed authorization, and fail-closed report remain
immutable. V17 proved MCL parent discovery and then incorrectly treated the
expanded parent product family as one calendar-gap-free roll chain. V18 keeps
every exact echo, bounded opaque group, interval, identity, cost, disk, and
destination gate, but assigns continuity semantics correctly: a parent query
must prove valid family mappings at both query boundaries, while only the
continuous ``<root>.v.0`` query may prove gap-free roll continuity. No raw
interval values, DBN rows, or download-capable API are exposed.
"""
...
from .micro_alpha_databento_preflight_v17 import (
    PLAN_PATH as PREDECESSOR_PLAN_PATH,
    REPORT_PATH as PREDECESSOR_REPORT_PATH,
    load_plan as load_v17_plan,
)
...
PREDECESSOR_PLAN_ID: Final = (
    "0b3711a03da42bdeebda0903d4fb33b6e1a7e5c7318f5cb43b89b781a25aff9d"
)
PREDECESSOR_PLAN_SHA256: Final = (
    "656f128fe9638454b56fa8799f3770ac91d9deff879ed7f81c42768d110dff19"
)
PREDECESSOR_REPORT_ID: Final = (
    "a2cc7312adcd4ee2b215a68db4821c5f573c8e23193e2560b9ccf2c924dd52f7"
)
PREDECESSOR_REPORT_SHA256: Final = (
    "a7f5891b8b2501ab3b00e67e65863e82deebc6d144971c383a3b961b2f5159ee"
)
PREDECESSOR_AUTHORIZATION_RECEIPT_ID: Final = (
    "71310872806b7f5c71e81fe0bd4333af93e0a449c380d95ec44328203b37e9c1"
)
...
PREDECESSOR_AUTHORIZATION_SHA256: Final = (
    "9f8d672c2f83d1cf54fe3bed0ee7217e0c50fdab43380f5bec2f7b6234aff7a3"
)
SCHEMA: Final = "apex_micro_databento_metadata_preflight/18.0.0"
REPORT_SCHEMA: Final = "apex_micro_databento_metadata_preflight_report/18.0.0"


def load_predecessor_failure(*, root: Path) -> dict[str, object]:
    """Verify exact v17 plan, failure report, and authorization evidence."""

    plan = load_v17_plan(root=root)
    ...
    if (
        ...
        raise IntegrityError("v17 predecessor evidence drifted")
    report = _object(report_path, "v17 metadata preflight failure report")
    expected_context = {
        "market": "MCL",
    ...
    if (
        ...
        or report.get("failure_code") != "POST_EFFECTIVE_COVERAGE_DRIFT"
        or report.get("exception_type") != "IntegrityError"
        or report.get("failed_provider_call_ordinal") != 8
        or report.get("failed_provider_operation") != "resolve"
        or report.get("failed_request_context") != expected_context
        or report.get("failed_validation_field") is not None
        or report.get("provider_call_total") != 8
        ...
        raise IntegrityError("v17 predecessor failure classification drifted")
    return report


def build_plan(*, root: Path) -> dict[str, object]:
    failure = load_predecessor_failure(root=root)
    predecessor = load_v17_plan(root=root)
    ...
    core.update(
        {
            ...
            "predecessor_execution": {
                ...
                "failure_code": "POST_EFFECTIVE_COVERAGE_DRIFT",
                "exception_type": "IntegrityError",
                "provider_call_total": 8,
                "failed_provider_operation": "resolve",
                "failed_provider_call_ordinal": 8,
                "failed_request_context": failure["failed_request_context"],
                "failed_validation_field": None,
                "automatic_retries": 0,
            ...
            "correction": {
                **dict(predecessor["correction"]),
                "reason": (
                    "V17_MCL_PARENT_FAMILY_INCORRECTLY_TREATED_AS_ONE_"
                    "CALENDAR_GAP_FREE_ROLL_CHAIN"
                ),
                "parent_family_mapping_semantics": (
                    "VALID_IDENTITIES_AND_QUERY_BOUNDARY_COVERAGE_"
                    "NO_SINGLE_ROLL_CHAIN_ASSERTION"
                ),
                "continuous_mapping_semantics": (
                    "GAP_FREE_CLIPPED_INTERVAL_UNION_PROVES_PROVIDER_"
                    "CONTINUOUS_ROLL_COVERAGE"
                ),
                "parent_result_intervals_claim_roll_continuity": False,
                "continuous_result_intervals_claim_roll_continuity": True,
    ...
    bindings.update(
        {
            ...
            "src/futures_rebuild/micro_alpha_databento_preflight_v18.py": (
                sha256_file(
                    root / "src/futures_rebuild/micro_alpha_databento_preflight_v18.py"
                )
            ),
        }
    )
    ...
    checks.pop("post_effective_gap_free_interval_union", None)
    checks.pop("post_effective_gap_free_clipped_interval_union", None)
    checks.update(
        {
            "parent_family_valid_identity_and_boundary_coverage": True,
            "parent_family_single_roll_chain_assumption_forbidden": True,
            "continuous_roll_gap_free_clipped_interval_union": True,
            "roll_continuity_claim_requires_continuous_symbology": True,
        }
    )
    core["checks"] = checks
...
def validate_plan(plan: Mapping[str, object], *, root: Path) -> dict[str, object]:
    ...
    if dict(plan) != expected:
        raise IntegrityError("Apex micro metadata preflight v18 plan drifted")
...
def load_plan(*, root: Path) -> dict[str, object]:
    return validate_plan(_object(root / PLAN_PATH, "v18 metadata preflight plan"), root=root)
...
def _symbology_summary(
    ...
    *,
    ...
    coverage_role: str = "CONTINUOUS_ROLL",
) -> dict[str, object]:
    if coverage_role not in {"PARENT_FAMILY", "CONTINUOUS_ROLL"}:
        raise IntegrityError("provider symbology coverage role is invalid")
    ...
    post_effective_boundary_coverage = False
    post_effective_gap_free = False
    if not allow_discovery_partial:
        if max(interval_end for _, interval_end in coverage_intervals) != end:
            raise IntegrityError("provider symbology post-effective coverage end drifted")
        post_effective_boundary_coverage = True
        if coverage_role == "CONTINUOUS_ROLL":
            cursor = query_start
            for interval_start, interval_end in sorted(coverage_intervals):
                if interval_start > cursor:
                    raise IntegrityError("provider symbology continuous roll coverage gap")
    ...
    disposition = (
        ...
        else "POST_EFFECTIVE_PARENT_FAMILY_BOUNDARY_COVERAGE_VERIFIED"
        if coverage_role == "PARENT_FAMILY"
        else "POST_EFFECTIVE_CONTINUOUS_ROLL_GAP_FREE_COVERAGE_VERIFIED"
    )
    return {
        ...
        "post_effective_boundary_coverage": post_effective_boundary_coverage,
        "post_effective_gap_free_coverage": post_effective_gap_free,
        "roll_continuity_claimed": (
            not allow_discovery_partial and coverage_role == "CONTINUOUS_ROLL"
        ),
        "parent_family_single_roll_chain_assumed": False,
...
def execute_preflight(
    *,
    ...
    """Consume one v18 authorization and run metadata-only operations."""
    ...
    boundary.assert_active_path(
        ...
        purpose="Apex micro v18 metadata report",
        subtree="state/unpublished_evidence/apex_micro_metadata_preflight_v18",
    )
    if report_path.exists():
        raise IntegrityError("Apex micro v18 metadata report is create-only")
    ...
    try:
        ...
        failure_code = (
            ...
            else "PARENT_FAMILY_BOUNDARY_COVERAGE_DRIFT"
            if "post-effective coverage start" in text
            or "post-effective coverage end" in text
            else "CONTINUOUS_ROLL_COVERAGE_DRIFT"
            if "continuous roll coverage gap" in text
