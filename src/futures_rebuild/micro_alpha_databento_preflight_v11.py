"""Bounded opaque-partial-flag-safe Apex micro metadata preflight successor.

The executed v10 plan, consumed authorization, and fail-closed report remain
immutable. V11 treats a bounded nonempty discovery-only ``partial`` string list
as an opaque boolean status under separately exact single-symbol request, echo,
and result-key bindings; neither contents nor exact cardinality are reported.
It derives the first mapping date, then re-resolves both parent and continuous
symbology from that date with empty ``partial`` and ``not_found`` lists
required. Dataset-start ambiguity and every later gap still fail closed. No
...
from .micro_alpha_databento_preflight_v10 import (
...
PREDECESSOR_PLAN_ID: Final = (
    "74175984ff728b098ae0dd47c1f8d2af0faaaecab094d7495a5595828aeeff6d"
)
PREDECESSOR_PLAN_SHA256: Final = (
    "161a5692c02becead99c900b6ca0bca0b1ea6ac773c4d2edbad51859c7e4f0cc"
)
...
PREDECESSOR_REPORT_SHA256: Final = (
    "a4df34e69431ff0e0c80ffea1223ab1e76b8f5b25df3aaf37a160b7a57c4196d"
)
PREDECESSOR_AUTHORIZATION_RECEIPT_ID: Final = (
    "9d83e1d4510c96b5e4fb73a8d780f512ebe2610c90afb20e534e0063fdedf7ef"
)
...
PREDECESSOR_AUTHORIZATION_SHA256: Final = (
    "c995261ba96049fe65caabde7838e2372264afe024c9f0582f279dd7daf1268a"
)
SCHEMA: Final = "apex_micro_databento_metadata_preflight/11.0.0"
REPORT_SCHEMA: Final = "apex_micro_databento_metadata_preflight_report/11.0.0"
MAXIMUM_PROVIDER_CALLS: Final = 375
MAXIMUM_OPAQUE_PARTIAL_ENTRIES: Final = 10_000
...
def load_predecessor_failure(*, root: Path) -> dict[str, object]:
    """Verify the exact v10 failure and its one-use authorization evidence."""
    ...
    if (
        ...
        raise IntegrityError("v10 metadata preflight evidence was not preserved byte-for-byte")
    plan = _object(plan_path, "v10 metadata preflight plan")
    plan_core = dict(plan)
    plan_id = plan_core.pop("plan_id", None)
    report = _object(report_path, "v10 metadata preflight report")
    ...
    if (
        ...
        raise IntegrityError("v10 metadata preflight failure evidence drifted")
    return report
...
def build_plan(*, root: Path) -> dict[str, object]:
    ...
    predecessor_plan = _object(
        root / PREDECESSOR_PLAN_PATH, "v10 metadata preflight plan"
    )
    ...
    if requests != predecessor_plan.get("requests"):
        raise IntegrityError("v11 market/schema request definitions drifted from v10")
    core: dict[str, object] = {
        ...
        "predecessor_execution": {
            ...
            "failed_call_inference": {
                "basis": (
                    "SEALED_V10_EXACT_SINGLE_SYMBOL_REQUEST_ECHO_AND_RESULT_KEY_"
                    "PLUS_PARTIAL_EXACT_STRING_LIST_REJECTED_BY_ONE_ENTRY_CEILING"
                ),
                "call_ordinal": 4,
                "operation": "resolve",
                "market": "MES",
                ...
                "stype_in": "parent",
                "local_defect": (
                    "OPAQUE_PROVIDER_PARTIAL_STATUS_MISINTERPRETED_AS_"
                    "EXACT_ONE_ENTRY_CARDINALITY"
                ),
                "affected_field_disposition": (
                    "V10_SANITIZED_PARTIAL_FIELD_NAME_ONLY_CONTENT_AND_COUNT_NOT_RECORDED"
                ),
                "inference_state": (
                    "BOUNDED_NONEMPTY_OPAQUE_STATUS_REQUIRES_INDEPENDENT_EXACT_"
                    "REQUEST_ECHO_RESULT_AND_DATE_BINDINGS"
                ),
        ...
        "correction": {
            "reason": "PROVIDER_PARTIAL_LIST_IS_AN_OPAQUE_STATUS_FLAG",
            "discovery_parent_partial_validation": (
                "EMPTY_OR_BOUNDED_NONEMPTY_EXACT_STRING_LIST_UNDER_EXACT_"
                "SINGLE_SYMBOL_REQUEST_ECHO_RESULT_AND_DATE_BINDINGS"
            ),
...
def validate_plan(plan: Mapping[str, object], *, root: Path) -> dict[str, object]:
    ...
    if dict(plan) != expected:
        raise IntegrityError("Apex micro metadata preflight v11 drifted")
...
def load_plan(*, root: Path) -> dict[str, object]:
    return validate_plan(_object(root / PLAN_PATH, "v11 metadata preflight plan"), root=root)
...
def _symbol_status(
    ...
    *,
    name: str,
    allow_discovery_partial: bool,
) -> bool:
    if type(value) is not list or any(type(item) is not str for item in value):
        raise IntegrityError(f"provider symbology {name} field is not an exact string list")
    if not value:
        return False
    if name == "partial" and allow_discovery_partial:
        if len(value) > MAXIMUM_OPAQUE_PARTIAL_ENTRIES:
            raise UnauthorizedOperation(
                "provider symbology partial entry ceiling exceeded"
            )
        return True
...
def execute_preflight(
    *,
    ...
    """Consume one v11 authorization and run metadata-only operations."""
    ...
    boundary.assert_active_path(
        ...
        purpose="Apex micro v11 metadata report",
        subtree="state/unpublished_evidence/apex_micro_metadata_preflight_v11",
    )
    if report_path.exists():
        raise IntegrityError("Apex micro v11 metadata report is create-only")
