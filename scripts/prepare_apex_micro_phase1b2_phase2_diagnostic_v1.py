"""Prepare-only CLI for the bounded first-interval Phase 2 diagnostic."""
...
from futures_rebuild.micro_alpha_phase1b2_phase2_diagnostic import (  # noqa: E402
    AUDIT_PATH,
...
def _require_committed_implementation() -> None:
    for path in IMPLEMENTATION_PATHS:
        ...
        if tracked.returncode != 0:
            raise SystemExit("every diagnostic implementation path must be committed")
    ...
    if changed.returncode != 0:
        raise SystemExit("diagnostic implementation paths must be committed")
...
def main() -> int:
    ...
    if command == "preview-plan":
        ...
        value = build_plan(root=ROOT, implementation_head=head)
        _check(PLAN_PATH, value, "Phase 2 diagnostic plan")
        path = PLAN_PATH
        ...
        _check(AUDIT_PATH, value, "Phase 2 diagnostic audit")
        path = AUDIT_PATH
    print(
        json.dumps(
            {
                ...
                "source_market": value.get("source", {}).get("market", value.get("source_market")),
                "source_schema": value.get("source", {}).get("schema", value.get("source_schema")),
                "source_year": value.get("source", {}).get("year", value.get("source_year")),
                "row_batches_opened": value.get("parquet_row_batches_opened", 0),
