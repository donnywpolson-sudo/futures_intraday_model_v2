"""Freeze a reconstruction-stable cleanup census for the v23 successor."""
...
if __package__:
    from scripts.prepare_safe_cleanup_candidate_census_v7 import (
        build_census as build_v7,
    )
else:
    from prepare_safe_cleanup_candidate_census_v7 import build_census as build_v7
...
V22_PLAN = Path("configs/apex_micro_tier01_phase1a_acquisition_plan_v22.json")
V22_AUDIT = Path(
...
DECLARED_CREATE_ONLY_OUTPUT_STATUS_PATHS = frozenset(
    {
        ...
        "state/unpublished_evidence/apex_micro_phase1a_acquisition_plan_v23/",
        "state/unpublished_evidence/safe_cleanup_candidate_census_v8/",
    }
)
...
def build_census(*, root: Path = ROOT, committed_head: str) -> dict[str, object]:
    predecessor = build_v7(root=root, committed_head=committed_head)
    core = dict(predecessor)
    core.pop("census_id", None)
    core["schema_version"] = "safe_cleanup_candidate_census/8.0.0"
    ...
    core["self_referential_output_exclusion"] = {
        ...
        "applies_only_to_create_only_v23_plan_audit_and_census_outputs": True,
    ...
    bindings.update(
        {
            ...
            V22_PLAN.as_posix(): sha256_file(root / V22_PLAN),
            V22_AUDIT.as_posix(): sha256_file(root / V22_AUDIT),
            V22_CENSUS.as_posix(): sha256_file(root / V22_CENSUS),
        }
    )
    core["bindings"] = dict(sorted(bindings.items()))
    core["superseded_v22_preparation"] = {
