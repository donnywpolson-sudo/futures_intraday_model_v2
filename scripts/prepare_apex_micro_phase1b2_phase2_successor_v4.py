"""Prepare-only CLI for the full inactive micro Phase 2 successor."""
...
from futures_rebuild.micro_alpha_phase1b2_phase2_successor import (  # noqa: E402
    AUDIT_PATH,
...
def _require_committed_implementation() -> None:
    for path in IMPLEMENTATION_PATHS:
        ...
        if tracked.returncode != 0:
            raise SystemExit("every Phase 2 successor implementation path must be committed")
    ...
    if changed.returncode != 0:
        raise SystemExit("Phase 2 successor implementation paths must be committed")
...
def main() -> int:
    ...
    if command == "preview-plan":
        ...
        value = build_plan(root=ROOT, implementation_head=head)
        _check(PLAN_PATH, value, "Phase 2 successor plan")
        path = PLAN_PATH
        ...
        _check(AUDIT_PATH, value, "Phase 2 successor audit")
        path = AUDIT_PATH
        ...
        json.dumps(
            {
                ...
                "coverage_cell_count": value.get("coverage_cell_count"),
