"""Freeze the lifecycle-safe five-schema diagnostic consolidation successor."""
...
OUTPUT = Path(
    ...
    "apex_micro_phase1b2_group_diagnostic_manifest_v3/manifest.json"
)
PREDECESSOR = Path(
    "state/unpublished_evidence/"
...
RECOMMENDED = (
    ...
    "scripts/prepare_apex_micro_phase1b2_group_diagnostic_manifest_v3.py",
    ...
    "tests/test_micro_alpha_phase1b2_group_diagnostic_manifest_v3.py",
...
def _postcommit_manifest() -> dict[str, object]:
    ...
    if set(
        ...
        raise IntegrityError("group diagnostic v3 commit differs from exact scope")
    value = json.loads(_committed_bytes(commit, OUTPUT.as_posix()))
    core = dict(value)
    if core.pop("manifest_id", None) != sha256_json(core):
        raise IntegrityError("group diagnostic v3 manifest identity drifted")
    ...
    for path in RECOMMENDED:
        if path == OUTPUT.as_posix():
            if records[path] != "SELF_HASHED_AT_WRITE":
                raise IntegrityError("group diagnostic v3 self marker drifted")
        elif hashlib.sha256(_committed_bytes(commit, path)).hexdigest() != records[path]:
            raise IntegrityError("group diagnostic v3 committed hash drifted")
    return value
...
def build_manifest() -> dict[str, object]:
    ...
    if _git("diff", "--cached", "--name-only"):
        raise IntegrityError("group diagnostic v3 manifest requires empty index")
    expected = set(RECOMMENDED) | set(PRESERVED_UNSTAGED)
    if not (ROOT / OUTPUT).exists():
        expected.remove(OUTPUT.as_posix())
    if _worktree_paths() != expected:
        raise IntegrityError("group diagnostic v3 worktree differs from exact scope")
    predecessor = _self_hashed(PREDECESSOR, "manifest_id", "superseded v2 manifest")
    ...
    if (
        ...
        != "29cc2c2b2c1adc64da03f51261dc3fa1ddfc02494446ee61f2696cc0adc41724"
        ...
        raise IntegrityError("group diagnostic v3 evidence drifted")
    if (
        ...
        raise IntegrityError("group diagnostic v3 Parquet drifted")
    records = [
        {
            "path": path,
    ...
    core: dict[str, object] = {
        "schema_version": "apex_micro_phase1b2_group_diagnostic_manifest/3.0.0",
        ...
        "supersession_reason": "LIFECYCLE_SAFE_POST_PLAN_TEST_SCOPE_ADDED",
