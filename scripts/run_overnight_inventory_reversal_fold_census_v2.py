"""Run only an approved parallel successor readiness census."""
...
from futures_rebuild.overnight_inventory_reversal_preexecution_census_v2 import (
    OPERATION,
    PLAN_PATH,
    execute_authorized_census_v2_once,
    load_census_v2_plan,
    required_scope_v2,
)
...
def main() -> None:
    ...
    required = required_scope_v2(root=ROOT, plan=plan)
    ...
    report = execute_authorized_census_v2_once(
        root=ROOT, boundary=boundary, receipt=receipt,
    )
    certificate = report["fold_readiness_certificate"]
    print(json.dumps({
        "status": "COMPLETED_UNPUBLISHED_PARALLEL_READINESS_CENSUS",
