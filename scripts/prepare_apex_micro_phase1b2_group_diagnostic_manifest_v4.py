"""Freeze the final lifecycle-safe five-schema diagnostic consolidation."""
...
from scripts import prepare_apex_micro_phase1b2_group_diagnostic_manifest_v3 as v3  # noqa: E402
...
PREDECESSOR = v3.OUTPUT
DIAGNOSTIC_PLAN = v3.DIAGNOSTIC_PLAN
DIAGNOSTIC_AUDIT = v3.DIAGNOSTIC_AUDIT
DIAGNOSTIC_REPORT = v3.DIAGNOSTIC_REPORT
DIAGNOSTIC_TERMINAL = v3.DIAGNOSTIC_TERMINAL
DIAGNOSTIC_PARQUET = v3.DIAGNOSTIC_PARQUET
PRESERVED_UNSTAGED = v3.PRESERVED_UNSTAGED
RECOMMENDED = (
    *v3.RECOMMENDED,
    "scripts/prepare_apex_micro_phase1b2_group_diagnostic_manifest_v4.py",
    OUTPUT.as_posix(),
    "tests/test_micro_alpha_phase1b2_group_diagnostic_manifest_v4.py",
)
...
def _postcommit_manifest() -> dict[str, object]:
    ...
    if set(
        ...
        raise IntegrityError("group diagnostic v4 commit differs from exact scope")
    value = json.loads(_committed_bytes(commit, OUTPUT.as_posix()))
    core = dict(value)
    if core.pop("manifest_id", None) != sha256_json(core):
        raise IntegrityError("group diagnostic v4 manifest identity drifted")
    ...
    for path in RECOMMENDED:
        if path == OUTPUT.as_posix():
            if records[path] != "SELF_HASHED_AT_WRITE":
                raise IntegrityError("group diagnostic v4 self marker drifted")
        elif hashlib.sha256(_committed_bytes(commit, path)).hexdigest() != records[path]:
            raise IntegrityError("group diagnostic v4 committed hash drifted")
    return value
...
def build_manifest() -> dict[str, object]:
    ...
    if _git("diff", "--cached", "--name-only"):
        raise IntegrityError("group diagnostic v4 manifest requires empty index")
    expected = set(RECOMMENDED) | set(PRESERVED_UNSTAGED)
    if not (ROOT / OUTPUT).exists():
        expected.remove(OUTPUT.as_posix())
    if _worktree_paths() != expected:
        raise IntegrityError("group diagnostic v4 worktree differs from exact scope")
    predecessor = _self_hashed(PREDECESSOR, "manifest_id", "superseded v3 manifest")
    plan = _self_hashed(DIAGNOSTIC_PLAN, "plan_id", "diagnostic plan")
    audit = _self_hashed(DIAGNOSTIC_AUDIT, "audit_id", "diagnostic audit")
    report = _self_hashed(DIAGNOSTIC_REPORT, "report_id", "diagnostic report")
    terminal = _self_hashed(DIAGNOSTIC_TERMINAL, "terminal_id", "diagnostic terminal")
    if (
        predecessor.get("manifest_id")
        != "c7134e1a65507ce70c7a0afceb2e6e663c8a578f316cb93c0928be96678a63dc"
        ...
        raise IntegrityError("group diagnostic v4 evidence drifted")
    diagnostic_sha = sha256_file(ROOT / DIAGNOSTIC_PARQUET)
    diagnostic_bytes = (ROOT / DIAGNOSTIC_PARQUET).stat().st_size
    if (
        diagnostic_sha != report.get("result", {}).get("output_sha256")
        or diagnostic_bytes != report.get("result", {}).get("output_bytes")
    ):
        raise IntegrityError("group diagnostic v4 Parquet drifted")
    records = [
        {
            "path": path,
    ...
    core: dict[str, object] = {
        "schema_version": "apex_micro_phase1b2_group_diagnostic_manifest/4.0.0",
        ...
        "superseded_manifest_id": predecessor["manifest_id"],
        "superseded_manifest_sha256": sha256_file(ROOT / PREDECESSOR),
        "supersession_reason": "FINAL_LIFECYCLE_AND_ALL_SCHEMA_TEST_SCOPE",
        ...
        "preserved_inactive_not_for_git": {
            ...
            "diagnostic_parquet_bytes": diagnostic_bytes,
            "diagnostic_parquet_sha256": diagnostic_sha,
