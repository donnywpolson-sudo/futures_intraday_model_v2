"""Freeze superseded v22 preparation and the corrected v23 successor change set."""
...
from futures_rebuild.micro_alpha_acquisition_v23 import (
    AUDIT_PATH as V23_AUDIT_PATH,
    PLAN_PATH as V23_PLAN_PATH,
    build_acquisition_plan,
)
from prepare_apex_micro_phase1a_acquisition_v22_supersession import (
    AUDIT as V22_AUDIT_PATH,
    CENSUS as V22_CENSUS_PATH,
    OUTPUT as V22_SUPERSESSION_PATH,
    PLAN as V22_PLAN_PATH,
    build_report as build_v22_supersession,
)
from prepare_safe_cleanup_candidate_census_v8 import (
    OUTPUT as V8_CENSUS_PATH,
    build_census,
)
...
OUTPUT = Path(
    ...
    "apex_micro_v23_acquisition_consolidation_manifest/manifest.json"
)
CATEGORIES = {
    "a_unexecuted_v22_preparation_and_supersession": (
        V22_PLAN_PATH.as_posix(),
        V22_AUDIT_PATH.as_posix(),
        V22_CENSUS_PATH.as_posix(),
        V22_SUPERSESSION_PATH.as_posix(),
        "scripts/prepare_apex_micro_phase1a_acquisition_v22_supersession.py",
    ),
    "b_reconstruction_stable_v23_successor": (
    ...
    "c_adversarial_tests_and_documentation": (
        ...
        "PROJECT_OUTLINE.md",
        "tests/test_micro_alpha_acquisition_v22_supersession.py",
        "tests/test_micro_alpha_acquisition_v23.py",
        "tests/test_safe_cleanup_candidate_census_v8.py",
    ),
    "d_consolidation_manifest_builder": (
        "scripts/prepare_apex_micro_v23_acquisition_consolidation_manifest.py",
    ),
...
def main() -> int:
    ...
    if any(
        (ROOT / path).exists()
        for path in (V23_PLAN_PATH, V23_AUDIT_PATH, V8_CENSUS_PATH)
    ):
        raise RuntimeError("v23 plan, audit, or cleanup census already exists")

    head = _git_value("rev-parse", "HEAD")
    supersession = _object(V22_SUPERSESSION_PATH)
    if build_v22_supersession(root=ROOT) != supersession:
        raise RuntimeError("v22 supersession does not reconstruct exactly")
    ...
    if plan_a != plan_b or cleanup_a != cleanup_b:
        raise RuntimeError("v23 plan or cleanup census is nondeterministic")
    destinations = [
        ROOT / str(item[key])
    ...
    core: dict[str, object] = {
        "schema_version": "apex_micro_v23_acquisition_consolidation_manifest/1.0.0",
        ...
        "verification": {
            "focused_v22_v23_tests_passed": 23,
            "complete_current_tests_passed": 508,
            "complete_high_risk_tests_passed": 1307,
            "final_dependency_documentation_tests_passed": 28,
        ...
        "authority_and_effects": {
            ...
            "v22_or_v23_download_performed": False,
    ...
    if output.exists():
        if output.read_bytes() != raw:
            raise RuntimeError("existing v23 consolidation manifest differs")
    else:
