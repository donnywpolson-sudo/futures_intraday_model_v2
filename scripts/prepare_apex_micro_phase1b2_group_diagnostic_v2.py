"""Prepare-only CLI for the bounded first-group Phase 2 diagnostic."""
...
from futures_rebuild.micro_alpha_phase1b2_group_diagnostic import (  # noqa: E402
    AUDIT_PATH,
...
def _require_committed_implementation() -> None:
    for path in IMPLEMENTATION_PATHS:
        ...
        if tracked.returncode != 0:
            raise SystemExit("every group diagnostic implementation path must be committed")
    ...
    if changed.returncode != 0:
        raise SystemExit("group diagnostic implementation paths must be committed")
...
def main() -> int:
    ...
    if command == "preview-plan":
        ...
        value = build_plan(root=ROOT, implementation_head=head)
        _check(PLAN_PATH, value, "Phase 2 first-group diagnostic plan")
        path = PLAN_PATH
        ...
        _check(AUDIT_PATH, value, "Phase 2 first-group diagnostic audit")
        path = AUDIT_PATH
    print(
        json.dumps(
            {
