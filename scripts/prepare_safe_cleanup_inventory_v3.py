...
OUTPUT = Path("state/unpublished_evidence/safe_cleanup_preparation_v3/plan.json")
PREDECESSOR_OUTPUT = Path("state/unpublished_evidence/safe_cleanup_preparation_v2/plan.json")
PREDECESSOR_SUPERSESSION = Path(
    "state/unpublished_evidence/safe_cleanup_preparation_v2_supersession.json"
)
PREDECESSOR_PLAN_ID = "876ec67f9cefcaa6ba7f99ea8c85bf5c78a0311a7a0ee7d76e9ac9f9de569907"
PREDECESSOR_PLAN_SHA256 = "b57f09570a341af0c8a13452734ef8da16f46f87c9cb797556ac39f0292cc74e"
...
def _worktree_paths(root: Path) -> list[str]:
    ...
    excluded = {OUTPUT.as_posix(), PREDECESSOR_OUTPUT.as_posix()}
    return sorted(path for path in output if path not in excluded)
...
def _protected_inventory(root: Path, relative: str) -> dict[str, object]:
    if relative == "state/unpublished_evidence":
        return {
            "exists": (root / relative).is_dir(),
            "kind": "DIRECTORY",
            "file_count": "NOT_CENSUSED_TO_AVOID_SELF_REFERENTIAL_PLAN_OUTPUT",
            "byte_count": "NOT_CENSUSED_TO_AVOID_SELF_REFERENTIAL_PLAN_OUTPUT",
        }
    return _inventory(root / relative)
...
def build_plan(*, root: Path = ROOT) -> dict[str, object]:
    ...
    if sha256_file(root / PREDECESSOR_OUTPUT) != PREDECESSOR_PLAN_SHA256:
        raise IntegrityError("superseded cleanup v1 plan was not preserved")
    predecessor = json.loads((root / PREDECESSOR_OUTPUT).read_text(encoding="utf-8"))
    supersession = json.loads(
        (root / PREDECESSOR_SUPERSESSION).read_text(encoding="utf-8")
    )
    if (
        predecessor.get("plan_id") != PREDECESSOR_PLAN_ID
        or supersession.get("plan_id") != PREDECESSOR_PLAN_ID
        or supersession.get("plan_sha256") != PREDECESSOR_PLAN_SHA256
        or supersession.get("execution_forbidden") is not True
    ):
        raise IntegrityError("cleanup v1 supersession evidence drifted")
    protected = [
        {
            "path": relative,
            **_protected_inventory(root, relative),
            "classification": (
    ...
    core: dict[str, object] = {
        "schema_version": "safe_data_project_cleanup_preparation/3.0.0",
        ...
        "superseded_predecessor": {
            ...
            "supersession_path": PREDECESSOR_SUPERSESSION.as_posix(),
            "reason": "SELF_OUTPUT_CHANGED_PROTECTED_EVIDENCE_ROOT_INVENTORY",
