"""Create only the immutable full-regular Alpha readiness plan."""
...
from futures_rebuild.alpha_ladder_full_regular_readiness import PLAN_PATH, build_plan, load_plan
...
def main() -> int:
    path = ROOT / PLAN_PATH
    if path.exists():
        raise FileExistsError("full-regular readiness plan already exists")
    ...
    print(json.dumps({
        ...
        "sha256": sha256_file(path), "state": plan["state"],
    }, sort_keys=True))
    return 0
